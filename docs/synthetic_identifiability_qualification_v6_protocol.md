# Identifiability-safe synthetic benchmark qualification v6
...
V6 uses fresh, mutually disjoint fixture, excluded-smoke, future-development, and confirmation registries. Every identifier found in the v1-v5 registry sources is blocked for all operations. The excluded smoke may run once after freeze; failure is retained.
...
The official test report uses the frozen repository interpreter and exactly two commands, v6 tests first and then `tests/`. It is the only test report admissible to the adjudicator. Traceability resolves real AST symbols, actually collected test node IDs, artifact paths, and gate fields. The final decision is regenerated in a clean staging directory from frozen official artifacts and must be byte-identical.

`V6_READY` requires every frozen gate. `REVISE` is used for a repairable package gate. `STOP` is used when the benchmark is structurally unidentifiable or requires oracle leakage or manufactured scoring. None authorizes a future-development or confirmation outcome.
