# Corrective alignment scientific protocol v2 / operational package v3 sample size
...
which is prospectively rounded to `0.952` in the configuration. Requiring both co-primary endpoints gives a conservative conjunctive-power lower bound of `1 - 2*(1-0.952) = 0.904` by the union bound. This calculation uses no construction or development outcome. The frozen exclusion rule is none: all 100 corpus seeds and all three paired model fits per corpus are required. There is no adaptive extension, exclusion, seed replacement, denominator reduction, rerun, or optional stopping; any missing, duplicate, malformed, interrupted, or failed unit aborts publication.
