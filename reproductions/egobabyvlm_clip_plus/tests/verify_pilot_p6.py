...
def main():
    ...
    require((c["model"]["hidden_size"],c["model"]["num_hidden_layers"],c["model"]["num_attention_heads"],c["model"]["intermediate_size"])==(768,12,12,3072),"not exact BERT-base")
    ...
    t=c["tokenizer"]; require(t["algorithm"]=="WordPiece" and t["vocab_size"]==380 and t["corpus"]=="training_split_only","tokenizer contract changed")
    ...
    require(a["status"]=="p6_complete" and a["tokenizer"]["vocab_size"]==380,"completion evidence invalid")
    require(not a["tokenizer"]["external_tokens"] and not a["tokenizer"]["fabricated_tokens"],"vocabulary provenance misstated")
    e=a["execution"]; require(e["health_checkpoint_step"]==e["resume_start_step"]==50 and e["final_optimizer_step"]==100 and e["distinct_process_resume_proven"],"execution state false")
    require(e["validation_runs"]==1 and not e["p4_loaded"] and not e["p5_or_dino_loaded"] and not e["p7_started"],"scope leakage claimed")
    v=a["verification"]; require(v["all_100_emitted_losses_finite"] and v["encoder_updated"] and v["mlm_head_updated"] and v["fresh_final_load_and_smoke_forward_passed"],"verification gates incomplete")
    ...
    print("Pilot P6 verification passed")
