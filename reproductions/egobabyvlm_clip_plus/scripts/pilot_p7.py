...
def paths(c,args):
    ...
    return run,rec
def verify(c,args,run,rec):
    require(not (rec/"completion.json").exists(),"P7_already_complete")
...
def train(args,c,run,manifest,lib):
    ...
    if resume:
        st=torch.load(args.resume,map_location="cpu",weights_only=False); require(st["global_update"]==65 and st["next_mode"]==modes[65],"resume_boundary")
...
def finalize(args,c,run,rec,manifest,lib):
    np,torch,*_=lib; st=torch.load(run/"checkpoints/step_130.pt",map_location="cpu",weights_only=False); require(st["global_update"]==130 and st["counts"]==c["schedule"]["exact_counts"],"final_counts")
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("phase",choices=("health","resume","finalize")); p.add_argument("--target",type=int,choices=(65,130));
    ...
    a=p.parse_args(); a.resume=None; c=config(a.config); run,rec=paths(c,a); manifest=verify(c,a,run,rec); lib=imports(a.source_root)
    ...
    else: finalize(a,c,run,rec,manifest,lib)
