...
from babyworld_lite.sensor_alignment_v5.adjudicator import REQUIRED_GATES, terminal_decision
from babyworld_lite.sensor_alignment_v5.benchmark import condition_audit, condition_view, generate_corpus
from babyworld_lite.sensor_alignment_v5.controls import (
    fit_direct_capacity_control,
    fit_oracle_control,
)
from babyworld_lite.sensor_alignment_v5.detector_runtime import (
    DetectorRuntime,
...
from babyworld_lite.sensor_alignment_v5.learner import (
...
CONFIG = ROOT / "configs/synthetic_identifiability_qualification_v5.yaml"
...
def test_v5_registries_are_disjoint_and_block_every_v1_v4_identifier(config):
    ...
    sets["prior_v1_v4"] = set(resolved["prior_v1_v4"])
    assert len(sets["prior_v1_v4"]) >= 150
    assert all(
...
def test_v5_firewall_blocks_prior_development_confirmation_for_all_operations(config):
    firewall = guard(config)
    for operation in OPERATIONS:
        for purpose, value in (
            ("future_development", 251001),
            ("confirmation_reserve", 259001),
            ("fixture_only", config["resolved_registries"]["prior_v1_v4"][0]),
        ):
...
def test_v5_corpus_crosses_required_factors_absence_null_and_noun_independence(corpus):
    audit = corpus.audit
...
def test_v5_visible_oracle_and_key_ledgers_are_separate(corpus):
...
def test_v5_matched_conditions_and_time_shifts_are_real(config, corpus, conditioned):
...
def test_v5_actual_frozen_v2_detector_class_and_function_are_invoked(
...
def test_v5_detector_capacity_is_helpful_imperfect_and_stratified(
...
def test_v5_order_invariant_metrics_and_fractional_ties(
...
def test_v5_sensor_free_gate_uses_genuine_fitted_outputs(config, corpus, conditioned):
    model, trace = fit_joint_cross_situational(
        conditioned[0]["absent_channel"],
        {},
        config,
...
def test_v5_iterations_competition_and_sensor_change_numeric_fit(
...
def test_v5_noun_sensor_weight_is_exactly_zero(config, conditioned, synchronized_evidence):
...
def test_v5_oracle_and_direct_controls_are_executed_not_constant_and_corruptible(
    config, corpus
):
    fitters = (
...
def test_v5_input_and_source_manifests_fail_on_one_byte_mutation(tmp_path):
...
def test_v5_traceability_breaks_each_reference_class(tmp_path):
...
def test_v5_adjudicator_fails_closed_and_uses_stop_only_for_structural_failure():
    all_pass = {name: True for name in REQUIRED_GATES}
    ready = terminal_decision({"gates": all_pass})
    assert ready["decision"] == "V5_READY"
...
def test_v5_complete_manifest_is_per_file_and_second_pass_verified(tmp_path):
...
def test_v5_v4_disposition_invalidates_promotion_without_rewriting_v4():
    disposition = (
        ROOT / "docs/synthetic_identifiability_qualification_v5_v4_disposition.md"
    ).read_text()
