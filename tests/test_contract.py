...
def test_phase4_preregistration_preserves_frozen_contract() -> None:
    ...
    assert config["status"] == "PHASE4_CORRECTED_ASSETS_PASS_LEARNER_EFFECTIVE_ENGINEERING_HEALTH_ATTEMPT_8_GIT_ABSENCE_BLOCKER_PRESERVED_CLEAN_TREE_FALLBACK_ATTEMPT_9_FROZEN_NO_NEW_OUTCOME"
    ...
    assert config["gates"]["learner_effective_runner_implementation_status"] == (
        "ATTEMPT_8_GIT_ABSENCE_BLOCKER_PRESERVED_CLEAN_TREE_FALLBACK_ATTEMPT_9_FROZEN_BEFORE_NEW_OUTCOME"
    )
    ...
    assert config["gates"]["learner_effective_public_model_inference_scope"] == (
        "ATTEMPT_9_GIT_FREE_CLEAN_TREE_ATTESTED_ENGINEERING_MICROHEALTH_ONLY_NO_SCIENTIFIC_METRICS"
    )
    assert config["gates"][
