...
def test_phase4_preregistration_preserves_frozen_contract() -> None:
    ...
    assert config["schema_version"] == 24
    assert config["status"] == "PHASE4_CORRECTED_ASSETS_PASS_LEARNER_EFFECTIVE_ENGINEERING_HEALTH_ATTEMPT_10_CROSS_PYTHON_AST_BLOCKER_PRESERVED_PORTABLE_AST_REPAIR_ATTEMPT_11_FROZEN_NO_NEW_OUTCOME"
    ...
    assert config["gates"]["learner_effective_runner_implementation_status"] == (
        "ATTEMPT_10_CROSS_PYTHON_AST_BLOCKER_PRESERVED_PORTABLE_AST_REPAIR_ATTEMPT_11_FROZEN_BEFORE_NEW_OUTCOME"
    )
    ...
    assert config["gates"]["learner_effective_public_model_inference_scope"] == (
        "ATTEMPT_11_PORTABLE_AST_REPAIRED_ENGINEERING_MICROHEALTH_ONLY_NO_SCIENTIFIC_METRICS"
    )
    assert config["gates"][
