...
def test_phase4_preregistration_preserves_frozen_contract() -> None:
    ...
    assert config["status"] == "PHASE4_CORRECTED_ASSETS_PASS_LEARNER_EFFECTIVE_ENGINEERING_HEALTH_ATTEMPT_11_FIXTURE_ROOT_BLOCKER_PRESERVED_READ_ONLY_FIXTURE_BIND_REPAIR_ATTEMPT_12_FROZEN_NO_NEW_OUTCOME"
    ...
    assert config["gates"]["learner_effective_runner_implementation_status"] == (
        "ATTEMPT_11_FIXTURE_ROOT_BLOCKER_PRESERVED_READ_ONLY_FIXTURE_BIND_REPAIR_ATTEMPT_12_FROZEN_BEFORE_NEW_OUTCOME"
    )
    ...
    assert config["gates"]["learner_effective_public_model_inference_scope"] == (
        "ATTEMPT_12_READ_ONLY_SEALED_FIXTURE_BIND_ENGINEERING_MICROHEALTH_ONLY_NO_SCIENTIFIC_METRICS"
    )
    assert config["gates"][
