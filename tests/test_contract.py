...
def test_phase4_qualification_wrapper_has_fail_closed_health_topology() -> None:
    ...
    assert '"$health_attempt" == "17"' in wrapper
...
def test_phase4_preregistration_preserves_frozen_contract() -> None:
    ...
    assert config["status"] == "PHASE4_CORRECTED_ASSETS_PASS_LEARNER_EFFECTIVE_ENGINEERING_HEALTH_ATTEMPT_16_RESOURCE_DISPATCH_BLOCKER_PRESERVED_ATTEMPT_17_REPAIR_FROZEN_NO_NEW_OUTCOME"
    ...
    assert config["gates"]["learner_effective_runner_implementation_status"] == (
        "ATTEMPT_16_RESOURCE_DISPATCH_BLOCKER_PRESERVED_ATTEMPT_17_REPAIR_FROZEN_BEFORE_NEW_OUTCOME"
    )
    ...
    assert config["gates"]["learner_effective_public_model_inference_scope"] == (
        "ATTEMPT_17_HISTORICAL_RESOURCE_DISPATCH_REPAIRED_ENGINEERING_MICROHEALTH_ONLY_NO_SCIENTIFIC_METRICS"
    )
    assert config["gates"][
