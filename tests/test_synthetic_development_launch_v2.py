...
import babyworld_lite.development_launch_v2.integrity as integrity
from babyworld_lite.development_launch_v2.adjudicator import (
    REQUIRED_GATES,
    authorization_payload,
    package_decision,
    verify_authorization,
)
from babyworld_lite.development_launch_v2.protocol import (
    IdentifierFirewall,
    IdentifierReference,
    load_config,
    manifest_for_files,
...
from babyworld_lite.development_launch_v2.integrity import (
    environment_record,
    finalize_package,
    write_complete_manifest,
)
from babyworld_lite.development_launch_v2.statistics import (
    average_model_replicates,
    bounded_paired_inference,
)
from babyworld_lite.development_launch_v2.study import execute_cohort
...
from scripts.run_synthetic_development_launch_v2 import (
    _verify_development_authorization,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/synthetic_development_launch_v2.yaml"
...
def test_firewall_blocks_development_without_authorization_and_confirmation(config):
    ...
    assert firewall.authorize(
        "generate",
        [IdentifierReference("corpus", 300001)],
...
def test_model_replicates_are_averaged_within_corpus(config):
    rows = []
    for model_seed, value in zip((300101, 300103, 300107), (0.6, 0.7, 0.8)):
        by_kind_presence = {
            kind: {
...
def test_confirmation_command_is_absent_and_claim_is_narrow(config):
    runner = (
        ROOT / "scripts/run_synthetic_development_launch_v2.py"
    ).read_text()
...
def test_exact_one_shot_command_and_output_are_frozen(config):
    ...
    assert "synthetic_development_v2_one_shot" in command
    assert not (ROOT / "output/synthetic_development_v2_one_shot").exists()
...
def test_environment_record_preserves_virtual_environment_launcher(config):
    ...
    assert Path(record["python_executable"]) == (
...
def test_negative_recompute_rejections_must_be_mutation_specific(
    ...
    source = (
        ...
        "development_launch_v2/__init__.py"
    )
    condition.parent.mkdir(parents=True)
    condition.write_text("{}\n")
    source.parent.mkdir(parents=True)
    source.write_text("PROTOCOL_ID = 'synthetic-development-launch-v2'\n")
    ...
    generic_source.write_text("PROTOCOL_ID = 'synthetic-development-launch-v2'\n")
...
def test_finalization_fails_closed_when_required_artifacts_are_missing(
    ...
    repository = tmp_path / "repository"
    output = repository / "output/synthetic_development_launch_package_v2"
...
def test_traceability_has_no_precreation_dependency():
    specification = json.loads(
        (
            ROOT / "docs/synthetic_development_launch_v2_traceability.json"
        ).read_text()
    )
    artifacts = {
        artifact
