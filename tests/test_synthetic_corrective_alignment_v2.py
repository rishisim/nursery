...
import babyworld_lite.corrective_alignment_v2.parallel as parallel_module
import babyworld_lite.corrective_alignment_v2.integrity as integrity_module
from babyworld_lite.corrective_alignment_v2.integrity import (
...
from babyworld_lite.corrective_alignment_v2.adjudicator import (
    AUTHORIZATION_FIELDS,
    PACKAGE_GATES,
    authorization_payload,
    verify_authorization,
)
from babyworld_lite.corrective_alignment_v2.statistics import dependence_diagnostics
from babyworld_lite.corrective_alignment_v2.generator import generate_corpus
from babyworld_lite.corrective_alignment_v2.learner import (
    disagreement_mechanism_qualification,
    fit_corrective_mil,
    predict_without_side,
)
from babyworld_lite.corrective_alignment_v2.parallel import (
...
from babyworld_lite.corrective_alignment_v2.protocol import (
...
import scripts.run_synthetic_corrective_alignment_v2 as runner


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/synthetic_corrective_alignment_v2.yaml"
...
def _make_prequalification_lock_fixture(tmp_path, monkeypatch, config):
    ...
    package = root / "output/synthetic_corrective_development_launch_package_v2"
    ...
    runner_path = root / "scripts/run_synthetic_corrective_alignment_v2.py"
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-outcome-registry-v2",
...
def _make_snapshot_receipt_fixture(base: Path):
    ...
    package.mkdir(parents=True)
    source = base / "source"
    runner_relative = "scripts/run_synthetic_corrective_alignment_v2.py"
    ...
    design_lock = {
        "schema_version": "nursery-corrective-prequalification-design-lock-v2",
    ...
    snapshot_config = snapshot / "configs/synthetic_corrective_alignment_v2.yaml"
...
def test_project_module_origins_are_confined_to_the_selected_source_root():
    expected = {
        "babyworld_lite.corrective_alignment_v2.protocol",
        "babyworld_lite.corrective_alignment_v2.generator",
        "babyworld_lite.corrective_alignment_v2.learner",
        "babyworld_lite.corrective_alignment_v2.statistics",
        "babyworld_lite.corrective_alignment_v2.parallel",
        "babyworld_lite.corrective_alignment_v2.adjudicator",
        "babyworld_lite.corrective_alignment_v2.integrity",
    }
...
def test_registry_allocation_is_fresh_disjoint_and_confirmation_sealed(
    ...
    assert all(330000 <= value <= 339999 for values in sets.values() for value in values)
    assert not any(
...
def test_corrective_disagreement_microcases_and_mutations(config):
    artifact = (
        EVIDENCE_ROOT
        / "output/synthetic_corrective_development_launch_package_v2/mechanism_qualification.json"
    )
...
def test_snapshot_receipt_rejects_schema_protocol_and_tracked_file_mutations(
    ...
    runner_path = snapshot2 / "scripts/run_synthetic_corrective_alignment_v2.py"
...
def test_parallel_micro_completion_rejects_mutated_or_partial_state(
    ...
    completion = {
        "schema_version": "nursery-corrective-parallel-micro-completion-v2",
        "status": "PASS",
        "prequalification_design_lock_sha256": sha256_file(
            package / "prequalification_design_lock.json"
        ),
...
def test_benchmark_validator_recomputes_gates_and_telemetry(
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-preservation-timing-v2",
            "status": "PASS",
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-preservation-completion-v2",
            "status": "PASS",
            "jobs": int(config["parallel"]["frozen_jobs"]),
...
def test_construction_attempt_receipt_rejects_mutated_hash_chain(tmp_path):
    ...
    receipt = {
        "schema_version": "nursery-corrective-construction-attempt-v2",
...
def _make_preflight_package(tmp_path: Path, monkeypatch, config) -> dict:
    ...
    package = repository / "output/synthetic_corrective_development_launch_package_v2"
    snapshot = package / "frozen_source_snapshot"
    ...
    snapshot_runner = snapshot / "scripts/run_synthetic_corrective_alignment_v2.py"
    snapshot_runner.parent.mkdir(parents=True)
    ...
    output = repository / "output/synthetic_corrective_development_v2_one_shot"
    staging = repository / "output/.synthetic_corrective_development_v2.staging"
    inner_staging = (
        repository / "output/..synthetic_corrective_development_v2.staging.cohort-staging"
    )
...
def _issue_fixture_development_capability(fixture: dict) -> tuple[object, dict]:
    ...
    claim = {
        "schema_version": "nursery-corrective-authorization-consumption-v2",
...
def test_capability_rechecks_authorization_and_worker_cannot_forge_chain(
    ...
    receipt_payload = {
        "schema_version": "nursery-corrective-capability-issuance-v2",
...
def test_preflight_binds_actual_paths_jobs_argv_and_replay(tmp_path, monkeypatch, config):
    ...
    alternate_snapshot = (
        tmp_path
        / "alternate-repository/output/synthetic_corrective_development_launch_package_v2/frozen_source_snapshot"
    )
    ...
    changed_argv[-1] = "1"
    monkeypatch.setattr(sys, "argv", [str(fixture["snapshot"] / "scripts/run_synthetic_corrective_alignment_v2.py"), *changed_argv])
    ...
    monkeypatch.setattr(sys, "argv", [str(fixture["snapshot"] / "scripts/run_synthetic_corrective_alignment_v2.py"), *fixture["argv"]])
    monkeypatch.setattr(
        runner,
        "reverify_prior_preservation",
...
def test_finalize_package_requires_sealed_evidence_proof(tmp_path, config):
    package = tmp_path / "output/synthetic_corrective_development_launch_package_v2"
    package.mkdir(parents=True)
    ...
    (snapshot / "configs/synthetic_corrective_alignment_v2.yaml").write_bytes(
        CONFIG.read_bytes()
    )
    (snapshot / "scripts/run_synthetic_corrective_alignment_v2.py").write_text(
...
def _complete_scheduler_validation_fixture(tmp_path, config, *, cohort_layout=False):
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-subprocess-success-v2",
            "status": "PASS",
            "unit_id": unit["unit_id"],
...
def test_scheduler_evidence_is_independently_recomputed_and_mutations_fail(
    ...
    immediate_relocation_report = validate_subprocess_scheduler_artifacts(
        ...
        expected_config_path=(
            relocated_repository / "configs/synthetic_corrective_alignment_v2.yaml"
        ),
...
def test_historical_worker_boundary_allows_only_exact_status_transition(
    ...
    config_path = repository / "configs/synthetic_corrective_alignment_v2.yaml"
...
def test_direct_subprocess_backend_has_no_multiprocessing_primitives(config):
    ...
    worker_source = (
        ROOT / "scripts/run_synthetic_corrective_alignment_v2_worker.py"
    ).read_text()
...
def test_real_subprocess_bootstrap_failure_is_semaphore_free_and_closed(
    ...
    result = subprocess.run(
        [
            ...
            "-s",
            "-P",
            str(ROOT / "scripts/run_synthetic_corrective_alignment_v2_worker.py"),
            "--request",
            str(request_path),
            "--request-sha256",
...
def test_official_test_report_validator_rejects_mutable_groups_and_bindings(
    ...
    (snapshot / "configs/synthetic_corrective_alignment_v2.yaml").write_text(
        "protocol: frozen\n", encoding="utf-8"
    )
    (snapshot / "scripts/run_synthetic_corrective_alignment_v2.py").write_text(
        "# frozen runner\n", encoding="utf-8"
    ...
    nodeids = [
        f"tests/test_synthetic_corrective_alignment_v2.py::{name}"
        for name in names
    ]
    group_rows = {
        group: {
...
def test_direct_live_rehearsal_execution_cannot_consume_frozen_attempt(
    ...
    fixture = _make_preflight_package(tmp_path, monkeypatch, config)
    frozen = load_config(
        fixture["snapshot"] / "configs/synthetic_corrective_alignment_v2.yaml",
        repository_root=fixture["snapshot"],
    )
    kwargs = {
        "repository_root": fixture["snapshot"],
...
def test_postfreeze_lifecycle_receipts_are_nonreplayable_and_exact(tmp_path):
    ...
    for source in (root, snapshot):
        config_path = source / "configs/synthetic_corrective_alignment_v2.yaml"
        runner_path = source / "scripts/run_synthetic_corrective_alignment_v2.py"
...
def test_parallel_artifacts_are_byte_identical_when_available():
    package = EVIDENCE_ROOT / "output/synthetic_corrective_development_launch_package_v2"
