...
import babyworld_lite.corrective_alignment_v4.parallel as parallel_module
import babyworld_lite.corrective_alignment_v4.integrity as integrity_module
from babyworld_lite.corrective_alignment_v4.integrity import (
...
from babyworld_lite.corrective_alignment_v4.adjudicator import (
    AUTHORIZATION_FIELDS,
    PACKAGE_GATES,
    authorization_payload,
    verify_authorization,
)
from babyworld_lite.corrective_alignment_v4.statistics import dependence_diagnostics
from babyworld_lite.corrective_alignment_v4.generator import (
    _event_intervals,
    _foil_map,
    _make_side_stream,
    _matched_background_intervals,
    _shift_stream,
    detect_event_or_null,
    generate_corpus,
)
from babyworld_lite.corrective_alignment_v4.learner import (
    _exact_window_event_or_null,
    _fit_presence_calibration,
    _sigmoid,
    disagreement_mechanism_qualification,
    fit_corrective_mil,
    predict_without_side,
)
from babyworld_lite.corrective_alignment_v4.parallel import (
...
from babyworld_lite.corrective_alignment_v4.protocol import (
...
import scripts.run_synthetic_corrective_alignment_v4 as runner


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/synthetic_corrective_alignment_v4.yaml"
...
@pytest.fixture()
def software_fixture_config(config):
    value = copy.deepcopy(config)
    value["resolved_registries"]["construction_micro"] = {
        "corpus": [-95001, -95003, -95005, -95007],
        "model": [-95201, -95203],
        "inference": [-95401],
    }
    return value
...
def _make_prequalification_lock_fixture(tmp_path, monkeypatch, config):
    ...
    package = root / "output/synthetic_corrective_development_launch_package_v4"
    package.mkdir(parents=True)
    config_path = root / "configs/synthetic_corrective_alignment_v4.yaml"
    ...
    runner_path = root / "scripts/run_synthetic_corrective_alignment_v4.py"
...
def _make_snapshot_receipt_fixture(base: Path):
    ...
    package.mkdir(parents=True)
    source = base / "source"
    runner_relative = "scripts/run_synthetic_corrective_alignment_v4.py"
    ...
    snapshot_config = snapshot / "configs/synthetic_corrective_alignment_v4.yaml"
    ...
    receipt = {
        "schema_version": "nursery-corrective-freeze-receipt-v4",
        "status": "FROZEN",
...
def test_project_module_origins_are_confined_to_the_selected_source_root():
    expected = {
        "babyworld_lite.corrective_alignment_v4.protocol",
        "babyworld_lite.corrective_alignment_v4.generator",
        "babyworld_lite.corrective_alignment_v4.learner",
        "babyworld_lite.corrective_alignment_v4.statistics",
        "babyworld_lite.corrective_alignment_v4.parallel",
        "babyworld_lite.corrective_alignment_v4.adjudicator",
        "babyworld_lite.corrective_alignment_v4.integrity",
    }
...
def test_registry_allocation_is_fresh_disjoint_and_confirmation_sealed(
    ...
    assert all(350000 <= value <= 359999 for values in sets.values() for value in values)
    assert not any(
...
def test_generated_corpus_separates_worker_inputs_keys_and_side(software_fixture_config):
    config = software_fixture_config
...
def test_action_geometry_varies_beyond_coordinate_permutation_and_is_shared(software_fixture_config):
    config = software_fixture_config
...
def test_train_evaluation_provenance_is_computed_and_detects_namespace_collision(software_fixture_config):
    config = software_fixture_config
    seed = config["resolved_registries"]["construction_micro"]["corpus"][0]
    corpus = generate_corpus(seed, config, _firewall(config))
...
def test_corrective_disagreement_microcases_and_mutations(config):
    artifact = (
        EVIDENCE_ROOT
        / "output/synthetic_corrective_development_launch_package_v4/mechanism_qualification.json"
    )
...
def test_model_seed_changes_initialization_order_and_fitted_state(software_fixture_config):
    config = software_fixture_config
    seed = config["resolved_registries"]["construction_micro"]["corpus"][0]
    corpus = generate_corpus(seed, config, _firewall(config))
    ...
    side_zero, side_zero_trace = fit_corrective_mil(
        ...
        model_seed=config["resolved_registries"]["construction_micro"]["model"][0],
        condition="synchronized",
        mutation="presence_side_disconnected",
    )
...
def test_prediction_is_side_free_and_rejects_sentinel(software_fixture_config):
    config = software_fixture_config
    seed = config["resolved_registries"]["construction_micro"]["corpus"][0]
    corpus = generate_corpus(seed, config, _firewall(config))
    learner_input = corpus["learner_input"]
...
def test_snapshot_receipt_rejects_schema_protocol_and_tracked_file_mutations(
    ...
    runner_path = snapshot2 / "scripts/run_synthetic_corrective_alignment_v4.py"
...
def test_ledger_requires_exact_count_chain_unit_and_references(
    software_fixture_config,
):
    config = software_fixture_config
    purpose = "construction_micro"
...
def test_firewall_and_operation_audit_reject_old_reserve_and_wrong_purpose_ids(
    tmp_path, config, software_fixture_config
):
    # Real retired/reserved identifiers are inspected only as static forged
    # ledger content; they are never passed through a guarded operation.
    static_values = {
    ...
    for label, value in static_values.items():
    ...
    fixture_config = copy.deepcopy(software_fixture_config)
    fixture_config["registries"]["permanently_quarantined_ranges"] = [
        *fixture_config["registries"]["permanently_quarantined_ranges"],
        [-99001, -99001],
    ]
    fixture_config["resolved_registries"]["development"]["corpus"] = [-96001]
    fixture_config["resolved_registries"]["confirmation_reserve"]["corpus"] = [
        -97001
    ]
    for label, value in {
        "dummy_quarantine": -99001,
        "dummy_confirmation_reserve": -97001,
        "dummy_wrong_purpose": -96001,
    }.items():
        firewall = _firewall(fixture_config)
        with pytest.raises(PermissionError):
            firewall.authorize(
                "generate",
    ...
    corpus = fixture_config["resolved_registries"]["construction_micro"][
        "corpus"
    ][0]
    valid = _firewall(fixture_config)
    valid.authorize(
        "generate",
...
def _make_preflight_package(tmp_path: Path, monkeypatch, config) -> dict:
    ...
    package = repository / "output/synthetic_corrective_development_launch_package_v4"
    snapshot = package / "frozen_source_snapshot"
    ...
    snapshot_runner = snapshot / "scripts/run_synthetic_corrective_alignment_v4.py"
    snapshot_runner.parent.mkdir(parents=True)
    ...
    output = repository / "output/synthetic_corrective_development_v4_one_shot"
    staging = repository / "output/.synthetic_corrective_development_v4.staging"
    inner_staging = (
        repository / "output/..synthetic_corrective_development_v4.staging.cohort-staging"
    )
...
def test_preflight_binds_actual_paths_jobs_argv_and_replay(tmp_path, monkeypatch, config):
    ...
    alternate_snapshot = (
        tmp_path
        / "alternate-repository/output/synthetic_corrective_development_launch_package_v4/frozen_source_snapshot"
    )
    ...
    changed_argv[-1] = "1"
    monkeypatch.setattr(sys, "argv", [str(fixture["snapshot"] / "scripts/run_synthetic_corrective_alignment_v4.py"), *changed_argv])
    ...
    monkeypatch.setattr(sys, "argv", [str(fixture["snapshot"] / "scripts/run_synthetic_corrective_alignment_v4.py"), *fixture["argv"]])
    monkeypatch.setattr(
        runner,
        "reverify_prior_preservation",
...
def test_finalize_package_requires_sealed_evidence_proof(tmp_path, config):
    package = tmp_path / "output/synthetic_corrective_development_launch_package_v4"
    package.mkdir(parents=True)
    ...
    (snapshot / "configs/synthetic_corrective_alignment_v4.yaml").write_bytes(
        CONFIG.read_bytes()
    )
    (snapshot / "scripts/run_synthetic_corrective_alignment_v4.py").write_text(
...
def test_finalize_package_revalidates_forged_factory_proof_and_config(
    ...
    snapshot_config = (
        package
        / "frozen_source_snapshot/configs/synthetic_corrective_alignment_v4.yaml"
    )
    snapshot_config.parent.mkdir(parents=True)
...
def _frozen_boundary_repository(tmp_path, config):
    ...
    package = (
        live_root
        / "output/synthetic_corrective_development_launch_package_v4"
    )
    snapshot = package / "frozen_source_snapshot"
    relatives = sorted(
        {
    ...
    frozen_config = load_config(
        snapshot / "configs/synthetic_corrective_alignment_v4.yaml",
...
def _complete_scheduler_validation_fixture(tmp_path, config, *, cohort_layout=False):
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-subprocess-success-v4",
            "status": "PASS",
            "unit_id": unit["unit_id"],
...
def test_scheduler_evidence_is_independently_recomputed_and_mutations_fail(
    ...
    immediate_relocation_report = validate_subprocess_scheduler_artifacts(
        ...
        expected_config_path=(
            relocated_repository / "configs/synthetic_corrective_alignment_v4.yaml"
        ),
...
def test_historical_worker_boundary_allows_only_exact_status_transition(
    ...
    config_path = repository / "configs/synthetic_corrective_alignment_v4.yaml"
        ...
        def __init__(self, argv, **_kwargs):
            ...
            if self.returncode == 1:
                ...
                write_json(
                    ...
                    {
                        "schema_version": "nursery-corrective-subprocess-failure-v4",
...
def test_real_subprocess_bootstrap_failure_is_semaphore_free_and_closed(
    ...
    result = subprocess.run(
        [
            ...
            "-s",
            "-P",
            str(ROOT / "scripts/run_synthetic_corrective_alignment_v4_worker.py"),
            "--request",
            str(request_path),
            "--request-sha256",
...
def test_parallel_requires_preparation_manifest_commitment_and_leaves_no_output(
    tmp_path, monkeypatch, software_fixture_config
):
    config = software_fixture_config
...
def test_official_test_report_validator_rejects_mutable_groups_and_bindings(
    ...
    (snapshot / "configs/synthetic_corrective_alignment_v4.yaml").write_text(
        "protocol: frozen\n", encoding="utf-8"
    )
    (snapshot / "scripts/run_synthetic_corrective_alignment_v4.py").write_text(
        "# frozen runner\n", encoding="utf-8"
    ...
    nodeids = [
        f"tests/test_synthetic_corrective_alignment_v4.py::{name}"
        for name in names
    ]
    group_rows = {
        group: {
    ...
    report = {
        "schema_version": "nursery-corrective-official-tests-v4",
        "status": "PASS",
        "executions": [
            {
                "command": commands[0],
...
def test_frozen_context_rejects_environment_drift(
    ...
    runner_path = snapshot / "scripts/run_synthetic_corrective_alignment_v4.py"
    runner_path.parent.mkdir(parents=True)
...
def test_direct_live_rehearsal_execution_cannot_consume_frozen_attempt(
    ...
    fixture = _make_preflight_package(tmp_path, monkeypatch, config)
    frozen = load_config(
        fixture["snapshot"] / "configs/synthetic_corrective_alignment_v4.yaml",
        repository_root=fixture["snapshot"],
    )
    kwargs = {
        "repository_root": fixture["snapshot"],
...
def test_postfreeze_lifecycle_receipts_are_nonreplayable_and_exact(tmp_path):
    ...
    for source in (root, snapshot):
        config_path = source / "configs/synthetic_corrective_alignment_v4.yaml"
        runner_path = source / "scripts/run_synthetic_corrective_alignment_v4.py"
...
def test_parallel_artifacts_are_byte_identical_when_available():
    package = EVIDENCE_ROOT / "output/synthetic_corrective_development_launch_package_v4"
...
def test_v3_scientific_core_is_exactly_inherited_from_v2():
    ...
    for name, expected in expected_sources.items():
        ...
        successor = ROOT / "babyworld_lite/corrective_alignment_v4" / name
...
def test_standard_library_bootstrap_writes_bound_import_failure_receipt(
    ...
    worker = source / "scripts/run_synthetic_corrective_alignment_v4_worker.py"
    parallel = source / "babyworld_lite/corrective_alignment_v4/parallel.py"
...
def _unregistered_v4_software_fixture_config():
    """Use negative, non-scientific IDs that cannot overlap any package registry."""
    value = _raw_v4_config()
...
def test_v4_guarded_event_and_matched_background_geometry_properties():
    value = _raw_v4_config()
    design = value["design"]
    shift = int(design["signed_shift_samples"])
    samples = int(design["raw_sensor_samples"])
    width_low, width_high = map(int, design["event_width"])
    gap_low, gap_high = map(int, design["event_gap"])
    for count in range(3, 6):
        for seed in range(256):
            events = _event_intervals(
                count, value, np.random.default_rng(seed * 17 + count)
            )
            background = _matched_background_intervals(events, value)
            widths = [end - start + 1 for start, end in events]
            background_widths = [end - start + 1 for start, end in background]
            gaps = [
                right[0] - left[1] - 1
                for left, right in zip(events, events[1:])
            ]
            assert len(events) == len(background) == count
            assert all(width_low <= width <= width_high for width in widths)
            assert background_widths == widths
            assert all(gap_low <= gap <= gap_high for gap in gaps)
            assert events[0][0] - shift >= 0
            assert events[-1][1] + shift < background[0][0]
            assert background[0][0] - shift > events[-1][1]
            assert background[-1][1] + shift < samples


def test_v4_signed_shifts_preserve_peak_energy_before_reassignment():
    value = _raw_v4_config()
    design = value["design"]
    shift = int(design["signed_shift_samples"])
    events = _event_intervals(5, value, np.random.default_rng(71))
    background = _matched_background_intervals(events, value)
    for target, null_background in ((0, None), (4, None), (None, 0), (None, 4)):
        stream = _make_side_stream(
            samples=int(design["raw_sensor_samples"]),
            intervals=events,
            background_intervals=background,
            target_index=target,
            null_background_index=null_background,
            informativity=1.0,
            noise=0.0,
            peak_range=[2.0, 2.0],
            rng=np.random.default_rng(912 + (target if target is not None else 20 + null_background)),
        )
        energy = math.fsum(stream)
        assert math.isclose(
            math.fsum(_shift_stream(stream, -shift)),
            energy,
            rel_tol=0.0,
            abs_tol=1e-12,
        )
...
def test_v4_matched_background_detector_has_no_count_or_width_null_bias():
    value = _raw_v4_config()
    rng = np.random.default_rng(20260717)
    rates = []
    for count in range(3, 6):
        events = _event_intervals(count, value, np.random.default_rng(500 + count))
        background = _matched_background_intervals(events, value)
        positives = 0
        trials = 2000
        for _ in range(trials):
            noise = np.abs(
                rng.normal(0.0, 0.10, size=value["design"]["raw_sensor_samples"])
            )
            detector = detect_event_or_null(noise, events, background)
            positives += int(float(detector["null_logit"]) > 0.0)
...
def test_v4_foil_matching_is_balanced_and_changes_both_dimensions():
    all_compositions = [(primitive, manner) for primitive in range(4) for manner in range(3)]
    for heldout_manners in (
        (0, 0, 1, 1),
        (0, 1, 2, 0),
        (2, 1, 0, 2),
    ):
        heldout = set(zip(range(4), heldout_manners))
        training = [value for value in all_compositions if value not in heldout]
        mapping = _foil_map(883, training)
        assert set(mapping) == set(training)
        assert set(mapping.values()) == set(training)
        assert all(
            source[0] != foil[0] and source[1] != foil[1]
            for source, foil in mapping.items()
        )


def test_v4_full_unregistered_corpus_requires_correction_and_hides_background_bank():
    value = _unregistered_v4_software_fixture_config()
    firewall = IdentifierFirewall(value, purpose="construction_micro")
    corpus = generate_corpus(-95001, value, firewall)
    audit = corpus["audit"]
    assert audit["correction_challenges_cover_every_training_composition"]
    assert audit["all_grounded_episodes_are_correction_challenges"]
    assert audit["every_training_composition_has_present_and_null_support"]
    assert audit["foil_map_is_bijective"]
    assert audit["foil_map_changes_both_action_dimensions"]
    assert audit["matched_background_proposal_banks"]
    assert audit["signed_shift_guard_bands_valid"]
    assert audit["realized_event_widths_within_frozen_range"]
    assert audit["realized_inter_event_gaps_within_frozen_range"]
    assert audit["all_condition_evidence_factorized"]
    assert all(
        "background_proposal_intervals" not in row and "side_stream" not in row
        for row in corpus["learner_input"]["training_episodes"]
    )
    protected = corpus["protected_adjudication"]["training_oracle"]
    assert all(bool(row["grounded"]) == bool(row["correction_challenge"]) for row in protected)
    assert all(
        list(row["composition"]) not in row["candidate_compositions"]
        for row in protected
        if not row["grounded"]
    )
    assert [entry["operation"] for entry in firewall.operations] == [
        "generate",
        "condition",
    ]


def test_v4_presence_calibration_and_exact_window_hand_contracts():
    value = _raw_v4_config()
    rows = [*((0.05, 0.02) for _ in range(8)), *((0.90, 0.98) for _ in range(8))]
    intercept, weight = _fit_presence_calibration(rows, value)
    low = _sigmoid(intercept + weight * 0.05)
    high = _sigmoid(intercept + weight * 0.90)
    assert weight > 0.0
    assert high - low >= value["qualification_gates"][
        "minimum_hand_case_presence_probability_separation"
    ]
    assert low <= value["qualification_gates"][
        "maximum_hand_case_present_null_probability"
    ]
    assert high >= value["qualification_gates"][
        "minimum_hand_case_null_null_probability"
    ]
    events = [{"start": 4, "end": 10}, {"start": 20, "end": 26}]
    event_posterior, null_posterior = _exact_window_event_or_null(events, 7.0)
    assert event_posterior.tolist() == [1.0, 0.0]
    assert null_posterior == 0.0
    event_posterior, null_posterior = _exact_window_event_or_null(events, 15.0)
    assert event_posterior.tolist() == [0.5, 0.5]
    assert null_posterior == 1.0


def test_v4_side_evidence_corrects_a_prespecified_wrong_semantic_ranking():
    value = _raw_v4_config()
    token = "hand-correction-token"
    episodes = []
    evidence = {}
    for repetition in range(8):
        episode_id = f"hand-correction-{repetition}"
        target = [0.97, 0.01, 0.01, 0.01]
        foil = [0.01, 0.97, 0.01, 0.01]
        episodes.append(
            {
                ...
                "events": [
                    {
                        "start": 4,
                        "end": 10,
                        "primitive_observation": target,
                        "manner_observation": [1 / 3, 1 / 3, 1 / 3],
                    },
                    {
                        "start": 20,
                        "end": 26,
                        "primitive_observation": foil,
                        "manner_observation": [1 / 3, 1 / 3, 1 / 3],
                    },
                ],
            }
    ...
    initial = {f"primitive|{token}": [0.02, 0.94, 0.02, 0.02]}
    results = {}
    for mutation in (
        "active",
        "semantic_side_zero",
        ...
        results[mutation] = (model, trace)
    assert int(
        np.argmax(results["active"][0].semantics[f"primitive|{token}"])
    ) == 0
    assert all(
        int(np.argmax(results[mutation][0].semantics[f"primitive|{token}"])) != 0
        for mutation in (
            "semantic_side_zero",
            "within_bag_permuted",
            "old_agreement_gate",
    ...
    assert results["active"][1]["side_changed_event_top_count"] > 0
    assert (
        results["semantic_update_disconnected"][1]["final_semantics_digest"]
        == results["semantic_update_disconnected"][1]["initial_semantics_digest"]
    )


def test_v4_learner_rejects_incomplete_or_nonfactorized_training_evidence():
    value = _raw_v4_config()
    episode = {
        "episode_id": "strict-evidence-hand",
        "utterance": {
            "items": [{"slot": "primitive", "token": "strict-token"}],
            "speech_time": 7.0,
        },
        "events": [
            {
                "start": 4,
                "end": 10,
                "primitive_observation": [0.97, 0.01, 0.01, 0.01],
                "manner_observation": [1 / 3, 1 / 3, 1 / 3],
            },
            {
                "start": 20,
                "end": 26,
                "primitive_observation": [0.01, 0.97, 0.01, 0.01],
                "manner_observation": [1 / 3, 1 / 3, 1 / 3],
            },
        ],
    }
    valid = {
        "strict-evidence-hand": {
            "event_logits": [4.0, -4.0],
            "null_logit": -5.0,
            "detector_input_present": True,
            "factorized_presence_event_logits": True,
    ...
    missing["extra-episode"] = copy.deepcopy(valid["strict-evidence-hand"])
    with pytest.raises(ValueError, match="episode set mismatch"):
        fit_corrective_mil(
            [episode], missing, value, model_seed=-8801, condition="hand"
        )
    nonfactorized = copy.deepcopy(valid)
    nonfactorized["strict-evidence-hand"]["factorized_presence_event_logits"] = False
    with pytest.raises(ValueError, match="malformed factorized"):
        fit_corrective_mil(
            [episode], nonfactorized, value, model_seed=-8801, condition="hand"
        )
    unavailable_active = copy.deepcopy(valid)
    unavailable_active["strict-evidence-hand"]["detector_input_present"] = False
    with pytest.raises(ValueError, match="unavailable detector"):
        fit_corrective_mil(
            [episode], unavailable_active, value, model_seed=-8801, condition="hand"
        )
