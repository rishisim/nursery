...
def test_phase4_seal_contract_is_identical_for_every_later_arm():
    ...
    assert result["status"] == (
        "CORRECTED_COMMON_ASSETS_PASS_PRIOR_RESULTS_PRESERVED_PUBLIC_ONLY_"
        "READINESS_DEVELOPMENT_SCIENTIFIC_NO_GO"
    )
...
def test_coverage_redesign_is_frozen_without_rewriting_prior_stop():
    ...
    assert proof["status"] == (
        "PUBLIC_ONLY_CALIBRATION_READINESS_DEVELOPMENT_SCIENTIFIC_NO_GO"
    )
    ...
    assert proof["schema_version"] == 38
