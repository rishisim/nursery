...
def test_phase4_seal_contract_is_identical_for_every_later_arm():
    ...
    assert result["status"] == "CORRECTED_COMMON_ASSETS_PASS_POST_GATE_DESCRIPTIVE_EXTENSION_STOPPED_CALIBRATION_NO_GO"
    ...
    assert result["post_gate_descriptive_extension"]["status"] == "STOPPED_FROZEN_CALIBRATION_CRITICAL_MISSINGNESS_NO_GO"
...
def test_coverage_redesign_is_frozen_without_rewriting_prior_stop():
    ...
    assert proof["status"] == "STOP_POST_GATE_DESCRIPTIVE_SYNTHETIC_1H_CALIBRATION_CRITICAL_MISSINGNESS_NO_GO"
    ...
    assert proof["calibration_C"]["local_generator_gate"].startswith("NO_GO_FROZEN")
