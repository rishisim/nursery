...
import babyworld_lite.development_launch_v3.integrity as integrity
...
from babyworld_lite.development_launch_v3.statistics import (
    average_model_replicates,
    bounded_paired_inference,
)
from babyworld_lite.development_launch_v3.study import execute_cohort
...
from scripts.run_synthetic_development_launch_v3 import (
    _verify_development_authorization,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/synthetic_development_launch_v3.yaml"
...
def test_firewall_blocks_development_without_authorization_and_confirmation(config):
    ...
    assert firewall.authorize(
        "generate",
        [IdentifierReference("corpus", 310001)],
...
def test_model_replicates_are_averaged_within_corpus(config):
    rows = []
    for model_seed, value in zip((310101, 310103, 310107), (0.6, 0.7, 0.8)):
        by_kind_presence = {
            kind: {
...
def test_final_runner_authorization_guard_verifies_manifest_and_zero_outcomes(
    ...
    for path, value in (
        ...
        (package / "traceability_validation.json", {"status": "PASS"}),
        (snapshot / "snapshot_manifest.json", {"files": []}),
        (
            snapshot / "nested/complete_file_manifest.json",
            {"role": "preserved nested evidence manifest"},
        ),
    ...
    files = [
        ...
        if path.is_file()
        and path != package / "complete_file_manifest.json"
    ]
...
def test_confirmation_command_is_absent_and_claim_is_narrow(config):
    runner = (
        ROOT / "scripts/run_synthetic_development_launch_v3.py"
    ).read_text()
...
def test_exact_one_shot_command_and_output_are_frozen(config):
    ...
    assert "synthetic_development_v3_one_shot" in command
    assert not (ROOT / "output/synthetic_development_v3_one_shot").exists()
...
def test_environment_record_preserves_virtual_environment_launcher(config):
    record = environment_record(ROOT, config)
    executable = Path(record["python_executable"])
    assert executable.is_file()
    configured = (ROOT / ".venv/bin/python").absolute()
    if configured.is_file():
        assert executable == configured
        assert (
            record["resolution_strategy"]
            == "configured_repository_virtual_environment"
        )
    else:
        assert record["resolution_strategy"] == (
            "active_virtual_environment_fallback"
        )
    assert Path(record["python_resolved_executable"]).is_file()
    completed = subprocess.run(
        [
            str(executable),
            "-c",
            "import numpy, pytest, yaml",
        ],
        cwd=ROOT,
        check=False,
        capture_output=True,
...
def test_negative_recompute_rejections_must_be_mutation_specific(
    tmp_path, monkeypatch
):
    output = tmp_path / "specific"
    condition = (
        output
        / "excluded_rehearsal/persisted_inputs/corpus_1/conditions/"
        "synchronized.jsonl"
    )
    source = (
        output
        / "frozen_source_snapshot/babyworld_lite/"
        "development_launch_v3/__init__.py"
    )
    condition.parent.mkdir(parents=True)
    condition.write_text("{}\n")
    source.parent.mkdir(parents=True)
    source.write_text("PROTOCOL_ID = 'synthetic-development-launch-v3'\n")

    def specific_command(_output, arguments, *, cwd, snapshot_root=None):
        if "mutated_inputs" in " ".join(arguments):
            message = "persisted input verification failed"
        else:
            message = "frozen source verification failed"
        return {
            "command": "fixture",
            "cwd": str(cwd),
            "exit_code": 1,
    ...
    monkeypatch.setattr(integrity, "_snapshot_command", specific_command)
    result = integrity.run_negative_recompute_tests(output)
    assert result["status"] == "PASS"
    assert result["rejections_are_mutation_specific"]
    ...
    generic_condition.parent.mkdir(parents=True)
    generic_condition.write_text("{}\n")
    generic_source.parent.mkdir(parents=True)
    generic_source.write_text("PROTOCOL_ID = 'synthetic-development-launch-v3'\n")

    def generic_command(_output, arguments, *, cwd, snapshot_root=None):
        return {
            "command": "fixture",
            "cwd": str(cwd),
            "exit_code": 1,
            "output": "ModuleNotFoundError: unrelated dependency",
            "snapshot_root": str(snapshot_root or ""),
        }

    monkeypatch.setattr(integrity, "_snapshot_command", generic_command)
    result = integrity.run_negative_recompute_tests(generic_output)
    assert result["status"] == "FAIL"
    assert not result["rejections_are_mutation_specific"]


def test_complete_manifest_has_canonical_digest_and_exact_file_set(tmp_path):
    (tmp_path / "evidence.json").write_text("{}\n")
    nested_manifest = tmp_path / "nested/complete_file_manifest.json"
    nested_manifest.parent.mkdir()
    nested_manifest.write_text('{"preserved": true}\n')
    ...
    actual = {
        ...
        if path.is_file()
        and path != tmp_path / "complete_file_manifest.json"
    }
    assert listed == actual
    assert "nested/complete_file_manifest.json" in listed


def test_finalization_fails_closed_when_required_artifacts_are_missing(
    config, tmp_path
):
    repository = tmp_path / "repository"
    output = repository / "output/synthetic_development_launch_package_v3"
    output.mkdir(parents=True)
    terminal = finalize_package(
        repository,
        output,
        config,
        {"protocol_id": config["protocol"]["id"], "requirements": []},
    )
    assert terminal["decision"] == "REVISE"
    assert (output / "package_terminal.json").is_file()
    assert (output / "complete_file_manifest.json").is_file()
    assert not (output / "DEVELOPMENT_LAUNCH_READY.json").exists()


def test_traceability_has_no_precreation_dependency():
    specification = json.loads(
        (
            ROOT / "docs/synthetic_development_launch_v3_traceability.json"
        ).read_text()
    )
    artifacts = {
        artifact
        for requirement in specification["requirements"]
        for artifact in requirement.get("artifacts", [])
    }
    assert artifacts.isdisjoint(
        {
            "DEVELOPMENT_LAUNCH_READY.json",
            "package_terminal.json",
            "terminal_regeneration.json",
            "complete_file_manifest.json",
            "traceability_validation.json",
        }
    )
