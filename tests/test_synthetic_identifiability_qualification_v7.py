...
from babyworld_lite.sensor_alignment_v7.adjudicator import REQUIRED_GATES, terminal_decision
from babyworld_lite.sensor_alignment_v7.benchmark import condition_audit, condition_view, generate_corpus
from babyworld_lite.sensor_alignment_v7.controls import (
    fit_direct_capacity_control,
    fit_oracle_control,
)
from babyworld_lite.sensor_alignment_v7.detector_runtime import (
    DetectorRuntime,
...
from babyworld_lite.sensor_alignment_v7.learner import (
...
CONFIG = ROOT / "configs/synthetic_identifiability_qualification_v7.yaml"
...
@pytest.fixture(scope="module")
def corpus(config):
    return generate_corpus(270001, config, guard(config), purpose="fixture_only")
...
def test_v7_registries_are_disjoint_and_block_every_v1_v6_identifier(config):
    ...
    sets["prior_v1_v6"] = set(resolved["prior_v1_v6"])
    assert len(sets["prior_v1_v6"]) >= 150
    assert all(
...
def test_v7_firewall_blocks_prior_development_confirmation_for_all_operations(config):
    firewall = guard(config)
    for operation in OPERATIONS:
        for purpose, value in (
            ("future_development", 271001),
            ("confirmation_reserve", 279001),
            ("fixture_only", config["resolved_registries"]["prior_v1_v6"][0]),
        ):
    ...
    assert firewall.authorize(
        "generate", [IdentifierReference("corpus", 270001)], purpose="fixture_only"
    )


def test_v7_corpus_crosses_required_factors_absence_null_and_noun_independence(corpus):
    audit = corpus.audit
...
def test_v7_visible_oracle_and_key_ledgers_are_separate(corpus):
...
def test_v7_matched_conditions_and_time_shifts_are_real(config, corpus, conditioned):
...
def test_v7_actual_frozen_v2_detector_class_and_function_are_invoked(
...
def test_v7_detector_capacity_is_helpful_imperfect_and_stratified(
...
def test_v7_order_invariant_metrics_and_fractional_ties(
    ...
    predictions = predict_prompts(
        model, prompts, config, guard(config), corpus_seed=270001, purpose="fixture_only"
    )
...
def test_v7_sensor_free_gate_uses_genuine_fitted_outputs(config, corpus, conditioned):
    model, trace = fit_joint_cross_situational(
        conditioned[0]["absent_channel"],
        {},
        config,
...
def test_v7_target_absent_training_is_required_for_every_null_subgroup(
    config, corpus, conditioned
):
...
def test_v7_iterations_competition_and_sensor_change_numeric_fit(
...
def test_v7_noun_sensor_weight_is_exactly_zero(config, conditioned, synchronized_evidence):
...
def test_v7_oracle_and_direct_controls_are_executed_not_constant_and_corruptible(
    config, corpus
):
    fitters = (
...
def test_v7_input_and_source_manifests_fail_on_one_byte_mutation(tmp_path):
...
def test_v7_traceability_breaks_each_reference_class(tmp_path):
...
def test_v7_adjudicator_fails_closed_and_uses_stop_only_for_structural_failure():
...
def test_v7_complete_manifest_is_per_file_and_second_pass_verified(tmp_path):
...
def test_v7_preserved_v5_v6_audit_records_revise_and_zero_outcomes():
    audit = json.loads(
        (
            ROOT
            / "docs/synthetic_identifiability_qualification_v7_v5_v6_audit.json"
        ).read_text()
    )
