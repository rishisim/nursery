...
from babyworld_lite.corrective_alignment_v4.adjudicator import (  # noqa: E402
    PACKAGE_GATES,
    construction_qualification_decision,
    verify_authorization,
)
from babyworld_lite.corrective_alignment_v4.integrity import (  # noqa: E402
    benchmark_report,
...
from babyworld_lite.corrective_alignment_v4.learner import (  # noqa: E402
    disagreement_mechanism_qualification,
)
from babyworld_lite.corrective_alignment_v4.parallel import (  # noqa: E402
...
from babyworld_lite.corrective_alignment_v4.protocol import (  # noqa: E402
    AuthorizationCapability,
    DevelopmentPreflightProof,
    IdentifierFirewall,
...
CONFIG_RELATIVE = "configs/synthetic_corrective_alignment_v4.yaml"
PACKAGE_RELATIVE = "output/synthetic_corrective_development_launch_package_v4"
TRACKED = (
    ...
    "babyworld_lite/corrective_alignment_v4/integrity.py",
    ...
    "tests/test_synthetic_corrective_alignment_v4.py",
    "docs/synthetic_corrective_alignment_v4_protocol.md",
    "docs/synthetic_corrective_alignment_v4_primary_sources.json",
    "docs/synthetic_corrective_alignment_v4_sample_size.md",
    "docs/synthetic_corrective_alignment_v4_traceability.json",
    "docs/synthetic_corrective_alignment_v4_research_rationale.md",
    "docs/synthetic_corrective_alignment_v4_threshold_rationale.md",
    "docs/synthetic_corrective_alignment_v4_brief_trace.json",
)
...
def _zero_outcome_registry(package: Path) -> dict:
    ...
    if not (
        ...
        == "nursery-corrective-outcome-registry-v4"
...
def _prequalification_pristine_paths(root: Path, config: dict) -> dict[str, str]:
    fields = {
        ...
        "authorization_consumption_root": "authorization_consumption_root",
...
def _official_prefreeze_prefixes() -> tuple[str, ...]:
    return (
        ...
        "preservation",
        "launch_seal",
        "development_authorization_consumed",
...
def _lifecycle_attempt_payload(
    ...
    runner_path = source_root / "scripts/run_synthetic_corrective_alignment_v4.py"
    snapshot_manifest = package / "frozen_source_snapshot/snapshot_manifest.json"
    freeze_receipt = package / "freeze_receipt.json"
    return {
        "schema_version": "nursery-corrective-lifecycle-attempt-v4",
        "status": status,
        "operation": operation,
...
def _validate_lifecycle_attempt_receipts(
    ...
    return {
        "schema_version": "nursery-corrective-lifecycle-attempt-validation-v4",
...
def _scientific_core_verification(root: Path, config: dict) -> dict:
    """Re-derive the prospective scientific-v3 core inside package v4."""
    sections = [
        "design",
        "learner",
        "analysis",
    ...
    checks = {
        "protocol_id_is_scientific_v3": config["protocol"]["id"]
        == "synthetic-corrective-alignment-v3",
        "package_version_is_v4": int(config["protocol"]["package_version"]) == 4,
    ...
    return {
        "schema_version": "nursery-corrective-scientific-core-verification-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "scientific_protocol_id": config["protocol"]["id"],
        "operational_package_version": 4,
...
def _create_prequalification_design_lock(
    ...
    lock = {
        "schema_version": "nursery-corrective-prequalification-design-lock-v4",
    ...
    *,
    ...
    if require_fixture_contracts:
        runner_row = next(
            ...
            if row["path"] == "scripts/run_synthetic_corrective_alignment_v4.py"
        )
        ...
        for name in selected_names:
            ...
            try:
                ...
                contract_pass = (
                    ...
                    == "nursery-corrective-scientific-execution-contract-v4"
                    ...
                    == "nursery-corrective-runtime-execution-contract-v4"
    ...
    return {
        "schema_version": "nursery-corrective-prequalification-lock-verification-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _repository_from_snapshot(snapshot: Path) -> Path:
    ...
    if resolved.parent.name != "synthetic_corrective_development_launch_package_v4":
...
def _predecessor_v2_tree_manifest(root: Path) -> dict:
    ...
    manifest = {
        "schema_version": "nursery-corrective-predecessor-tree-manifest-v4",
...
def _predecessor_v2_evidence(root: Path) -> tuple[dict, dict]:
    ...
    audit = {
        "schema_version": "nursery-corrective-predecessor-failure-audit-v4",
        "status": "PASS_PRESERVED_FAILED_NO_OUTCOME",
        "predecessor_path": predecessor_relative,
...
def _verify_or_create_predecessor_v2_evidence(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-predecessor-preservation-verification-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
_PREDECESSOR_V3_TREE_DIGEST = (
    "c296c408bb397c45965eff05525f1ce85cf612162cd6176c047f43fc0aceb4fb"
)
_PREDECESSOR_V3_LOCK_SHA256 = (
    "3188f1b41a21bfd88afb246f4162435b5d6e3abe3f61e9fc2361e1901913045c"
)
_PREDECESSOR_V3_DECISION_SHA256 = (
    "82eacd164706011ea84d965de4128ad71261e6375de51142e72daa7a3d947cdc"
)
_PREDECESSOR_V3_OUTCOME_SHA256 = (
    "9e28fcec041be64cd72a7aba4880d3ddc837e68f0408f1eaa0689aeec01de597"
)
_PREDECESSOR_V3_OPERATIONAL_IDENTIFIERS_DIGEST = (
    "74990480795cc2b5b4db89cdc85e8fd0f175b69e7283791326c4a27f9e962a22"
)


def _predecessor_v3_tree_manifest(root: Path) -> dict:
    predecessor = root / "output/synthetic_corrective_development_launch_package_v3"
    metadata = predecessor.lstat()
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
        raise RuntimeError("retired v3 package must be a real directory")
    rows = [
        {
            "path": ".",
            "type": "directory",
    ...
    for relative in tree_directory_paths(predecessor):
        ...
        child = path.lstat()
        if stat.S_ISLNK(child.st_mode) or not stat.S_ISDIR(child.st_mode):
            raise RuntimeError(f"retired v3 directory changed type: {relative}")
        rows.append(
            {
                "path": relative,
                "type": "directory",
    ...
    manifest = {
        "schema_version": "nursery-corrective-predecessor-tree-manifest-v4",
        "root": "output/synthetic_corrective_development_launch_package_v3",
        "rows": rows,
        "row_count": len(rows),
        "file_count": sum(row["type"] == "file" for row in rows),
        "directory_count_including_root": sum(
        ...
        "regular_file_bytes": sum(int(row.get("bytes", 0)) for row in rows),
        "symlink_count": 0,
        "other_node_count": 0,
        "digest_scheme": "sha256-compact-json-array-of-sorted-type-mode-path-rows",
        "digest": hashlib.sha256(
    ...
    if (
        manifest["digest"] != _PREDECESSOR_V3_TREE_DIGEST
        or manifest["row_count"] != 806
        or manifest["file_count"] != 695
        or manifest["directory_count_including_root"] != 111
        or manifest["regular_file_bytes"] != 204_108_929
    ):
        raise RuntimeError("retired v3 full tree changed")
    return manifest


def _predecessor_v3_evidence(root: Path) -> tuple[dict, dict]:
    predecessor_relative = "output/synthetic_corrective_development_launch_package_v3"
    predecessor = root / predecessor_relative
    first = _predecessor_v3_tree_manifest(root)
    second = _predecessor_v3_tree_manifest(root)
    if first != second:
        raise RuntimeError("retired v3 tree changed across double census")

    stable_digests: dict[str, str] = {}

    def stable_json(relative: str) -> dict:
        data, _metadata = _read_confined_stable_regular_bytes(
            predecessor / relative, predecessor
        )
        stable_digests[relative] = hashlib.sha256(data).hexdigest()
        return json.loads(data.decode("utf-8"))

    lock_path = predecessor / "prequalification_design_lock.json"
    decision_path = predecessor / "construction_qualification_decision.json"
    outcome_path = predecessor / "outcome_registry.json"
    lock = stable_json("prequalification_design_lock.json")
    decision = stable_json("construction_qualification_decision.json")
    outcome = stable_json("outcome_registry.json")
    if (
        stable_digests["prequalification_design_lock.json"]
        != _PREDECESSOR_V3_LOCK_SHA256
        ...
        raise RuntimeError("retired v3 design lock changed")
    source_paths = [
        str(row["path"])
        for row in lock["tracked_nonconfig_manifest"]["files"]
    ]
    if manifest_for_paths(root, source_paths) != lock["tracked_nonconfig_manifest"]:
        raise RuntimeError("retired v3 locked source bytes changed")
    config_path = root / "configs/synthetic_corrective_alignment_v3.yaml"
    if (
        sha256_file(config_path)
        != "483faf7d5ad33e2cd9895d78d2aaa3f27cadd91a35c589551ac20ba0605c9eda"
        or sha256_file(config_path) != lock["prefreeze_config_sha256"]
    ):
        raise RuntimeError("retired v3 config changed")
    if (
        stable_digests["construction_qualification_decision.json"]
        != _PREDECESSOR_V3_DECISION_SHA256
        or decision.get("schema_version")
        != "nursery-corrective-construction-decision-v2"
        or decision.get("status") != "FAIL"
        or decision.get("scientific_inference_suppressed") is not True
        or int(decision.get("development_outcome_count", -1)) != 0
        or int(decision.get("confirmation_outcome_count", -1)) != 0
    ):
        raise RuntimeError("retired v3 construction decision changed")
    if (
        stable_digests["outcome_registry.json"]
        != _PREDECESSOR_V3_OUTCOME_SHA256
        or int(outcome.get("development_outcome_count", -1)) != 0
        or int(outcome.get("confirmation_outcome_count", -1)) != 0
        or outcome.get("authorization_consumed") is not False
        or outcome.get("development_output_exists") is not False
        or outcome.get("confirmation_output_exists") is not False
    ):
        raise RuntimeError("retired v3 outcome registry changed")

    _predecessor_v1_evidence(root)
    _predecessor_v2_evidence(root)
    historical_predecessor_hashes = {
        "PREDECESSOR_V1_FILE_MANIFEST.json": (
            "cc342ea3b5246db3eec9d823196c15790329b7e3f5e1b45716d2109414ad287d"
        ),
        "PREDECESSOR_V1_DIRECTORY_MANIFEST.json": (
            "f0a18c3583dddda2806cd2c786c7596423965fa884b5b1d2111f87f424aa3518"
        ),
        "PREDECESSOR_V2_TREE_MANIFEST.json": (
            "34cf5be8b30750a30661adb6b603d8461c09d99849347301f432605c91b4e8e2"
        ),
        "PREDECESSOR_V2_FAILURE_AUDIT.json": (
            "94b71fd08d862c1ee21f719b7242ceba1fe25ed003e737f48b249ab9de27d05c"
        ),
    }
    for relative in historical_predecessor_hashes:
        stable_json(relative)
    if any(
        stable_digests[relative] != expected
        for relative, expected in historical_predecessor_hashes.items()
    ):
        raise RuntimeError("retired v3 transitive predecessor evidence changed")
    ...
    ledger_paths = sorted(
        ...
        if row["type"] == "file" and Path(str(row["path"])).name in ledger_names
    )
    identifiers: set[int] = set()
    ...
    def collect_identifiers(value: object) -> None:
        if isinstance(value, Mapping):
            if (
                ...
                identifiers.add(int(value["value"]))
            for child in value.values():
    ...
    for relative in ledger_paths:
        collect_identifiers(stable_json(relative))
    if (
        len(ledger_paths) != 70
        or len(identifiers) != 61
        or canonical_digest(sorted(identifiers))
        != _PREDECESSOR_V3_OPERATIONAL_IDENTIFIERS_DIGEST
        or any(not 340000 <= identifier <= 349999 for identifier in identifiers)
    ):
        raise RuntimeError("retired v3 operation ledger census changed")
    ...
    critical_absences = [
        ...
        f"{predecessor_relative}/development_authorization_consumed.json",
        f"{predecessor_relative}/development_capability_issued.json",
        "output/.synthetic_corrective_development_launch_seal_v3.staging",
        "output/synthetic_corrective_development_v3_one_shot",
        "output/.synthetic_corrective_development_v3.staging",
        "output/..synthetic_corrective_development_v3.staging.cohort-staging",
    ]
    absence_checks = {
        relative: _lstat_absent(root / relative) for relative in critical_absences
    }
    if not all(absence_checks.values()):
        raise RuntimeError("retired v3 downstream/outcome path appeared")
    failed_gates = sorted(
        name for name, passed in decision["gates"].items() if not bool(passed)
    )
    if failed_gates != [
        "all_informativeness_and_manipulation_gates",
        "model_training_stochasticity_is_numerically_relevant",
        "presence_null_effects_not_identical",
    ]:
        raise RuntimeError("retired v3 failed-gate inventory changed")
    audit = {
        "schema_version": "nursery-corrective-predecessor-failure-audit-v4",
        "status": "PASS_PRESERVED_FAILED_CONSTRUCTION_NO_OUTCOME",
        "predecessor_path": predecessor_relative,
        "predecessor_package_version": 3,
        "successor_package_version": 4,
        "immutable_by_policy_and_rehash": True,
        "os_immutable_flag_claimed": False,
        "retry_forbidden": True,
        "failure_stage": "construction_qualification_decision",
        "failure_reason": "frozen construction gates did not all pass",
        "failed_construction_gates": failed_gates,
        "tree_manifest_digest": first["digest"],
        "tree_double_census_identical": True,
        "design_lock_sha256": stable_digests[
            "prequalification_design_lock.json"
        ],
        "construction_decision_sha256": stable_digests[
            "construction_qualification_decision.json"
        ],
        "outcome_registry_sha256": stable_digests["outcome_registry.json"],
        "operation_ledger_file_count": len(ledger_paths),
        "structured_operational_identifier_count": len(identifiers),
        "structured_operational_identifiers_digest": canonical_digest(
            sorted(identifiers)
        ),
        "all_structured_operational_identifiers_in_retired_v3_namespace": True,
        "guarded_281xxx_references": 0,
        "guarded_289xxx_references": 0,
        "rehearsal_development_confirmation_operationally_untouched": True,
        "critical_lstat_absences": absence_checks,
        "transitive_v1_preservation": "PASS",
        "transitive_v2_preservation": "PASS",
        "failed_namespace_permanently_quarantined": [340000, 349999],
        "fresh_successor_namespace_required": [350000, 359999],
        "development_outcome_count": 0,
        "confirmation_outcome_count": 0,
        "authorization_consumed": False,
        "scientific_outcome": False,
...
def _verify_or_create_predecessor_v3_evidence(
    root: Path,
    package: Path,
    *,
    create: bool,
) -> dict:
    tree_manifest, audit = _predecessor_v3_evidence(root)
    tree_path = package / "PREDECESSOR_V3_TREE_MANIFEST.json"
    audit_path = package / "PREDECESSOR_V3_QUALIFICATION_FAILURE_AUDIT.json"
    if create:
        if not _lstat_absent(tree_path) or not _lstat_absent(audit_path):
            raise RuntimeError("partial predecessor v3 evidence already exists")
        write_json(tree_path, tree_manifest)
        audit = {**audit, "tree_manifest_sha256": sha256_file(tree_path)}
        write_json(audit_path, audit)
    observed_tree = read_json(tree_path)
    ...
    checks = {
        "full_tree_manifest_exact": observed_tree == tree_manifest,
        "qualification_failure_audit_exact": observed_audit == expected_audit,
        "predecessor_currently_reverified": True,
    }
    return {
        "schema_version": "nursery-corrective-predecessor-preservation-verification-v4",
        ...
        "checks": checks,
        "tree_manifest_sha256": sha256_file(tree_path),
        "qualification_failure_audit_sha256": sha256_file(audit_path),
        "development_outcome_count": 0,
        "confirmation_outcome_count": 0,
    }
...
def _initialize_package(root: Path) -> Path:
    ...
    if _lstat_absent(package):
        ...
        predecessor_v3 = _verify_or_create_predecessor_v3_evidence(
            root,
            staging,
            create=True,
        )
        if predecessor_v3["status"] != "PASS":
            raise RuntimeError(
                f"predecessor v3 preservation creation failed: {predecessor_v3}"
            )
    ...
    if stat.S_ISLNK(package_metadata.st_mode) or not stat.S_ISDIR(
        ...
        raise RuntimeError("corrective v4 package must be a real directory")
    ...
    if predecessor_v3["status"] != "PASS":
        raise RuntimeError(
            f"predecessor v3 preservation verification failed: {predecessor_v3}"
        )
    return package
...
def _worker_launcher_preflight(root: Path, config: dict) -> dict:
    ...
    expected_worker = (
        root / "scripts/run_synthetic_corrective_alignment_v4_worker.py"
    ).absolute()
    ...
    parallel_path = (
        root / "babyworld_lite/corrective_alignment_v4/parallel.py"
    ).absolute()
    ...
    with tempfile.TemporaryDirectory(prefix="corrective-v4-bootstrap-") as temporary:
    ...
    if not (
        ...
        == "nursery-corrective-worker-bootstrap-probe-v4"
        and probe.get("status") == "PASS"
        and probe.get("source_root") == str(root)
        and probe.get("parallel_module")
        == str(root / "babyworld_lite/corrective_alignment_v4/parallel.py")
    ...
    return {
        "schema_version": "nursery-corrective-worker-launcher-preflight-v4",
        "status": "PASS",
        "argv": command,
        "cwd": str(root),
...
def _consume_micro_attempt(
    ...
    payload = {
        "schema_version": "nursery-corrective-micro-attempt-v4",
        ...
        "runner_sha256": sha256_file(
            root / "scripts/run_synthetic_corrective_alignment_v4.py"
        ),
        ...
        "predecessor_v3_qualification_failure_audit_sha256": sha256_file(
            package / "PREDECESSOR_V3_QUALIFICATION_FAILURE_AUDIT.json"
        ),
        "predecessor_v3_tree_manifest_sha256": sha256_file(
            package / "PREDECESSOR_V3_TREE_MANIFEST.json"
        ),
        "development_outcome_count": 0,
        "confirmation_outcome_count": 0,
        "scientific_inference_suppressed": True,
...
def _validate_micro_attempt_receipt(
    ...
    expected_receipt_fields = {
        ...
        "predecessor_v3_qualification_failure_audit_sha256",
        "predecessor_v3_tree_manifest_sha256",
        "development_outcome_count",
        "confirmation_outcome_count",
        "scientific_inference_suppressed",
        "non_replayable",
    ...
    checks = {
        ...
        and receipt.get("schema_version") == "nursery-corrective-micro-attempt-v4"
        and receipt.get("status")
        == "ONE_MICRO_ATTEMPT_CONSUMED_BEFORE_LAUNCHER_PREFLIGHT"
        and int(receipt.get("package_version", -1)) == 4
        ...
        == sha256_file(root / "scripts/run_synthetic_corrective_alignment_v4.py")
        and receipt.get("worker_sha256")
        == sha256_file(root / "scripts/run_synthetic_corrective_alignment_v4_worker.py"),
        ...
        and receipt.get("predecessor_v3_qualification_failure_audit_sha256")
        == sha256_file(
            package / "PREDECESSOR_V3_QUALIFICATION_FAILURE_AUDIT.json"
        )
        and receipt.get("predecessor_v3_tree_manifest_sha256")
        == sha256_file(package / "PREDECESSOR_V3_TREE_MANIFEST.json"),
    ...
    return {
        "schema_version": "nursery-corrective-micro-attempt-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _micro(root: Path, config: dict) -> dict:
    ...
    try:
        ...
        write_json(
            ...
            {
                "schema_version": (
                    "nursery-corrective-worker-launcher-preflight-failure-v4"
                ),
    ...
    write_json(
        completion_path,
        {
            "schema_version": "nursery-corrective-parallel-micro-completion-v4",
...
def _validate_parallel_micro_attempt_1(
    ...
    checks = {
        ...
        == "nursery-corrective-parallel-micro-completion-v4",
    ...
    return {
        "schema_version": "nursery-corrective-parallel-micro-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _validate_construction_attempt_receipt(
    ...
    *,
    ...
    checks = {
        ...
        == "nursery-corrective-construction-attempt-v4",
    ...
    return {
        "schema_version": "nursery-corrective-construction-attempt-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "receipt_sha256": sha256_file(receipt_path),
    }
...
def _construction(root: Path, config: dict) -> dict:
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-construction-attempt-v4",
    ...
    merged_results = read_json(
        cohort_root / "adjudication/merged_worker_results.json"
    )
...
def _frozen_context(snapshot_value: str) -> tuple[Path, Path, dict]:
    ...
    if Path(__file__).resolve() != (
        snapshot / "scripts/run_synthetic_corrective_alignment_v4.py"
    ).resolve():
    ...
    if (
        ...
        or predecessor_v3.get("status") != "PASS"
    ):
...
def _excluded_rehearsal(snapshot_value: str, jobs: int) -> dict:
    ...
    completion = {
        "schema_version": "nursery-corrective-excluded-rehearsal-completion-v4",
        "status": "PASS",
...
def _strict_development_preflight(
    *,
    ...
    expected_runner = snapshot / "scripts/run_synthetic_corrective_alignment_v4.py"
    ...
    expected_paths = {
        ...
        "resolved_consumption_root": repository
        / str(config["paths"]["authorization_consumption_root"]),
        "resolved_consumed_authorization_path": repository
        / str(config["paths"]["consumed_authorization"]),
    }
    ...
    predecessor_v3_live_verification = _verify_or_create_predecessor_v3_evidence(
        repository,
        package,
        create=False,
    )
    ...
    if (
        ...
        or predecessor_v3_live_verification.get("status") != "PASS"
    ...
    for value in (
        ...
        expected_paths["resolved_consumption_root"],
        expected_paths["resolved_consumed_authorization_path"],
    ...
    parsed_contract = {
        ...
        "consumption_root": str(
            expected_paths["resolved_consumption_root"].resolve()
        ),
        "consumed_authorization": str(
            expected_paths["resolved_consumed_authorization_path"].resolve()
        ),
        "jobs": int(jobs),
        ...
        "predecessor_v3_live_verification_digest": canonical_digest(
            predecessor_v3_live_verification
        ),
    ...
    proof = _issue_development_preflight_proof(
        ...
        package_complete_manifest_sha256=sha256_file(
            package / "launch_seal/complete_file_manifest.json"
        ),
        ...
        consumption_root=expected_paths["resolved_consumption_root"],
        consumed_authorization_path=expected_paths[
            "resolved_consumed_authorization_path"
        ],
        outcome_registry_path=package / "outcome_registry.json",
...
class _DevelopmentPublicationProof:
    __slots__ = (
        ...
        "predecessor_v3_verification_digest",
...
def _live_development_publication_preservation(
    ...
    checks = {
        ...
        "predecessor_v3": predecessor_v3.get("status") == "PASS",
    ...
    return {
        ...
        "predecessor_v3_verification_digest": canonical_digest(predecessor_v3),
...
def _issue_development_publication_proof(
    *,
    ...
    return _DevelopmentPublicationProof(
        ...
        predecessor_v3_verification_digest=live_preservation[
            "predecessor_v3_verification_digest"
        ],
...
def _verify_development_publication_proof(
    ...
    *,
    ...
    if (
        ...
        or proof.predecessor_v3_verification_digest
        != live_preservation["predecessor_v3_verification_digest"]
...
def _publish_development_staging(
    *,
    ...
    publication = {
        "schema_version": "nursery-corrective-scientific-publication-v4",
        "status": "COMPLETE",
        ...
        "predecessor_v3_verification_digest": (
            publication_proof.predecessor_v3_verification_digest
        ),
        "prior_preservation_verification_digest": (
    ...
    if (
        ...
        != publication_proof.predecessor_v2_verification_digest
        or final_preservation["predecessor_v3_verification_digest"]
        != publication_proof.predecessor_v3_verification_digest
...
def _consume_development_authorization(
    *,
    repository: Path,
    snapshot: Path,
    ...
    """Durably consume the one-shot authority before any guarded operation."""
    package = snapshot.parent
    registry_path = package / "outcome_registry.json"
    consumption_root = Path(str(authorization["resolved_consumption_root"]))
    consumed_authorization = Path(
        str(authorization["resolved_consumed_authorization_path"])
    )
    claim_path = Path(str(authorization["resolved_claim_path"]))
    receipt_path = Path(str(authorization["resolved_capability_receipt_path"]))
    if not (
        consumption_root.parent == package
        and consumed_authorization.parent == consumption_root
        and claim_path.parent == consumption_root
        and receipt_path.parent == consumption_root
        and len(
            {
                consumed_authorization.name,
                claim_path.name,
                receipt_path.name,
            }
        )
        ...
        raise PermissionError("authorization consumption path layout mismatch")
    for path in (
        consumption_root,
        ...
        require_lstat_absent(path)
        _no_symlink_ancestry(path, stop=repository)
    ...
    if not (
        ...
        raise PermissionError("authorization cannot consume a non-pristine registry")
    authorization_bytes, authorization_metadata = (
        _read_confined_stable_regular_bytes(
    ...
    if (
        json.loads(authorization_bytes.decode("utf-8")) != authorization
        or not stat.S_ISREG(authorization_metadata.st_mode)
    ):
        raise PermissionError("authorization bytes changed before consumption")

    # The registry transition is the first durable monotonic marker.  Even if
    # the process stops immediately afterward, deleting partial attempt files
    # cannot restore a launchable state.
    write_json(
        registry_path,
        {**registry, "authorization_consumed": True},
        overwrite=True,
    )
    _fsync_directory(package)
    consumption_root.mkdir(mode=0o700)
    _fsync_directory(package)
    atomic_write_bytes(consumed_authorization, authorization_bytes)
    if sha256_file(consumed_authorization) != hashlib.sha256(
        authorization_bytes
    ).hexdigest():
        raise PermissionError("consumed authorization copy mismatch")
    original_authorization_path.unlink()
    _fsync_directory(original_authorization_path.parent)
    if original_authorization_path.exists() or original_authorization_path.is_symlink():
        raise PermissionError("original authorization remained after consumption")
    claim = {
        "schema_version": "nursery-corrective-authorization-consumption-v4",
        "status": "ATTEMPT_CONSUMED",
        "authorization_digest": authorization["authorization_digest"],
        "original_authorization_path": str(original_authorization_path),
        "resolved_authorization_path": str(consumed_authorization),
        ...
        "resolved_capability_receipt_path": str(receipt_path),
        "resolved_consumption_root": str(consumption_root),
    ...
    if tree_directory_paths(consumption_root) or tree_file_paths(
        consumption_root
    ) != sorted([consumed_authorization.name, claim_path.name]):
        raise PermissionError("authorization consumption tree is not exact")
    return claim_path, consumed_authorization
...
def _run_development(
    ...
    parsed_contract_digest = canonical_digest(parsed_contract)
    claim_path, _consumed_authorization_path = _consume_development_authorization(
...
def _design_contract_validation(
    ...
    brief = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v4_brief_trace.json"
    )
    ...
    sources = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v4_primary_sources.json"
    )
    threshold_text = (
        snapshot / "docs/synthetic_corrective_alignment_v4_threshold_rationale.md"
    ...
    return {
        "schema_version": "nursery-corrective-design-contract-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "categories": {
...
def _rehearsal_contract_validation(
    ...
    checks = {
        ...
        == sha256_file(
            snapshot / "scripts/run_synthetic_corrective_alignment_v4_worker.py"
        )
        ...
        == "nursery-corrective-cohort-summary-v4"
        and summary.get("status") == "PASS"
    ...
    return {
        "schema_version": "nursery-corrective-rehearsal-contract-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _pristine_state_validation(root: Path, package: Path, config: dict) -> dict:
    ...
    checks = {
        ...
        and registry.get("schema_version") == "nursery-corrective-outcome-registry-v4"
        and int(registry.get("development_outcome_count", -1)) == 0
        and int(registry.get("confirmation_outcome_count", -1)) == 0
        and registry.get("development_output_exists") is False
        and registry.get("confirmation_output_exists") is False
    ...
    return {
        "schema_version": "nursery-corrective-pristine-state-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _traceability(package: Path, snapshot: Path) -> dict:
    specification = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v4_traceability.json"
    )
    expected_status = {
        "micro_attempt_consumed.json": (
            "ONE_MICRO_ATTEMPT_CONSUMED_BEFORE_LAUNCHER_PREFLIGHT"
    ...
    specification_checks = {
        ...
        == "nursery-corrective-traceability-v4",
    ...
    return {
        "schema_version": "nursery-corrective-traceability-validation-v4",
        "status": "PASS" if passed else "FAIL",
...
def _development_command_walltime_estimate(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-development-command-walltime-v4",
...
def _validate_development_command_walltime_estimate(
    ...
    return {
        "schema_version": "nursery-corrective-development-walltime-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _validate_preservation_completion(package: Path, config: dict) -> dict:
    ...
    checks = {
        ...
        == "nursery-corrective-preservation-timing-v4"
        ...
        and completion.get("schema_version")
        == "nursery-corrective-preservation-completion-v4"
        and completion.get("status") == "PASS"
    ...
    return {
        "schema_version": "nursery-corrective-preservation-completion-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _finalize(root: Path, config: dict) -> dict:
    ...
    adjudication_input = {
        "schema_version": "nursery-corrective-package-adjudication-input-v4",
        "gates": gates,
        "contradictions": [],
    ...
    if args.command == "run-development":
        ...
        if args.command == "micro":
            ...
            write_json(
                ...
                {
                    "schema_version": "nursery-corrective-preservation-timing-v4",
            ...
            write_json(
                ...
                {
                    "schema_version": "nursery-corrective-preservation-completion-v4",
                    "status": "PASS" if result.get("status") == "PASS" else "FAIL",
                    "jobs": int(config["parallel"]["frozen_jobs"]),
