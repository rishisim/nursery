...
def render_performance(metrics: dict, output: Path) -> None:
    ...
    figure.suptitle("Real, Hailuo, and LTX performance — one public 10-second source clip", x=0.02, y=0.98, ha="left", fontsize=16, weight="bold")
...
def render_cost(costs: dict, projection: dict, output: Path) -> dict:
    ...
    figure.suptitle("Direct video-generation API cost projections", x=0.02, y=0.98, ha="left", fontsize=16, weight="bold")
    figure.text(0.02, 0.91, "Observed accepted-request charges projected linearly from 10 seconds to one usable hour; descriptor, local compute, storage, QA, labor, and paid failures are excluded.", ha="left", va="top", fontsize=9.5, color="#4B5563")
    figure.savefig(output, dpi=180, facecolor="white")
    plt.close(figure)
    return per_hour
