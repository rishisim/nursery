...
def extract_frame(ffmpeg, source, timestamp, target):
    target.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
    subprocess.run([ffmpeg, "-nostdin", "-hide_banner", "-loglevel", "error", "-ss", f"{timestamp:.3f}", "-i", str(source), "-frames:v", "1", "-vf", "scale=512:-2", "-y", str(target)], check=True)
    os.chmod(target, 0o600)
...
def build_temporal(root: Path, cfg, checkpoint):
    ...
    for u in utterances:
        ...
        selected=[]
        for pool,n in ((same_session,3),(same_child,2),(other,2)):
            selected += sorted(pool,key=lambda x:digest([u["utterance_key"],x["utterance_key"]]))[:n]
        ...
        for off in cfg["temporal"]["frame_offsets_seconds"]:
            ...
            if not target.exists(): extract_frame(ffmpeg,root/u["file"],window_start+off,target)
            frames.append(str(target.relative_to(root/"temporal")));frame_hashes.append(file_digest(target))
        candidates=[u]+selected
        order=sorted(range(8),key=lambda i:digest([u["utterance_key"],candidates[i]["utterance_key"]]))
        rows.append({"query_key":u["utterance_key"],"frames":frames,"candidate_texts":[candidates[i]["text"] for i in order],"positive_index":order.index(0),"child_key":u["child_key"],"session_key":u["session_key"]})
    if not rows: raise RuntimeError("E_TEMPORAL_SUPPORT")
    ...
    return root/"temporal/restricted_manifest.json", {"frame_duplicate_count":len(frame_hashes)-len(set(frame_hashes)),"row_count":len(rows)}


def seal(root: Path, cfg, lexical_paths, temporal_path, audits):
    ...
    lexical_commitment=digest(lexical_rows); temporal_commitment=file_digest(temporal_path)
    common={arm:{"lexical":lexical_commitment,"temporal":temporal_commitment} for arm in cfg["sealing"]["all_later_arms"]}
    record={"schema_version":1,"status":"PASS","lexical_commitment":lexical_commitment,"temporal_commitment":temporal_commitment,"common_asset_references":common,"test_assets_may_steer_later_work":False,"generated_images_not_held_out_real":True,"temporal_model_derived_no_referent_ground_truth":True,"audits":audits}
    write_json(root/"sealed/restricted_phase4_seal.json",record)
    compact={k:v for k,v in record.items() if k not in {"audits"}}
...
def main():
    ...
    if rank==0:
        ...
        seal(root,cfg,lexical,temporal,audits)
