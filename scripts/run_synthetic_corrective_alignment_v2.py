...
from babyworld_lite.corrective_alignment_v2.adjudicator import (  # noqa: E402
    PACKAGE_GATES,
    construction_qualification_decision,
    verify_authorization,
)
from babyworld_lite.corrective_alignment_v2.integrity import (  # noqa: E402
    benchmark_report,
...
from babyworld_lite.corrective_alignment_v2.learner import (  # noqa: E402
    disagreement_mechanism_qualification,
)
from babyworld_lite.corrective_alignment_v2.parallel import (  # noqa: E402
...
from babyworld_lite.corrective_alignment_v2.protocol import (  # noqa: E402
...
PACKAGE_RELATIVE = "output/synthetic_corrective_development_launch_package_v2"
TRACKED = (
    ...
    "babyworld_lite/corrective_alignment_v2/adjudicator.py",
    "babyworld_lite/corrective_alignment_v2/integrity.py",
    ...
    "docs/synthetic_corrective_alignment_v2_protocol.md",
    "docs/synthetic_corrective_alignment_v2_primary_sources.json",
    "docs/synthetic_corrective_alignment_v2_sample_size.md",
    "docs/synthetic_corrective_alignment_v2_traceability.json",
    "docs/synthetic_corrective_alignment_v2_research_rationale.md",
    "docs/synthetic_corrective_alignment_v2_threshold_rationale.md",
    "docs/synthetic_corrective_alignment_v2_brief_trace.json",
)
...
def _zero_outcome_registry(package: Path) -> dict:
    ...
    if not (
        ...
        == "nursery-corrective-outcome-registry-v2"
...
def _lifecycle_attempt_payload(
    ...
    runner_path = source_root / "scripts/run_synthetic_corrective_alignment_v2.py"
    snapshot_manifest = package / "frozen_source_snapshot/snapshot_manifest.json"
    freeze_receipt = package / "freeze_receipt.json"
    return {
        "schema_version": "nursery-corrective-lifecycle-attempt-v2",
        "status": status,
        "operation": operation,
...
def _validate_lifecycle_attempt_receipts(
    ...
    return {
        "schema_version": "nursery-corrective-lifecycle-attempt-validation-v2",
    ...
    *,
    ...
    if require_fixture_contracts:
        runner_row = next(
            ...
            if row["path"] == "scripts/run_synthetic_corrective_alignment_v2.py"
        )
        ...
        for name in selected_names:
            ...
            try:
                ...
                contract_pass = (
                    ...
                    == "nursery-corrective-scientific-execution-contract-v2"
                    ...
                    == "nursery-corrective-runtime-execution-contract-v2"
    ...
    return {
        "schema_version": "nursery-corrective-prequalification-lock-verification-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _repository_from_snapshot(snapshot: Path) -> Path:
    ...
    if resolved.parent.name != "synthetic_corrective_development_launch_package_v2":
...
def _initialize_package(root: Path) -> Path:
    ...
    if stat.S_ISLNK(package_metadata.st_mode) or not stat.S_ISDIR(
        ...
        raise RuntimeError("corrective v2 package must be a real directory")
...
def _validate_parallel_micro_attempt_1(
    ...
    checks = {
        ...
        == "nursery-corrective-parallel-micro-completion-v2",
    ...
    return {
        "schema_version": "nursery-corrective-parallel-micro-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _validate_construction_attempt_receipt(
    ...
    *,
    ...
    checks = {
        ...
        == "nursery-corrective-construction-attempt-v2",
    ...
    return {
        "schema_version": "nursery-corrective-construction-attempt-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _excluded_rehearsal(snapshot_value: str, jobs: int) -> dict:
    ...
    completion = {
        "schema_version": "nursery-corrective-excluded-rehearsal-completion-v2",
        "status": "PASS",
...
def _strict_development_preflight(
    *,
    ...
    expected_runner = snapshot / "scripts/run_synthetic_corrective_alignment_v2.py"
...
def _publish_development_staging(
    *,
    ...
    publication = {
        "schema_version": "nursery-corrective-scientific-publication-v2",
        "status": "COMPLETE",
...
def _design_contract_validation(
    ...
    brief = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v2_brief_trace.json"
    )
    ...
    sources = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v2_primary_sources.json"
    )
    threshold_text = (
        snapshot / "docs/synthetic_corrective_alignment_v2_threshold_rationale.md"
    ...
    return {
        "schema_version": "nursery-corrective-design-contract-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "categories": {
...
def _rehearsal_contract_validation(
    ...
    checks = {
        ...
        == sha256_file(
            snapshot / "scripts/run_synthetic_corrective_alignment_v2_worker.py"
        )
        ...
        == "nursery-corrective-cohort-summary-v2"
        and summary.get("status") == "PASS"
    ...
    return {
        "schema_version": "nursery-corrective-rehearsal-contract-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _pristine_state_validation(root: Path, package: Path, config: dict) -> dict:
    ...
    checks = {
        ...
        and registry.get("schema_version") == "nursery-corrective-outcome-registry-v2"
        and int(registry.get("development_outcome_count", -1)) == 0
        and int(registry.get("confirmation_outcome_count", -1)) == 0
        and registry.get("development_output_exists") is False
        and registry.get("confirmation_output_exists") is False
    ...
    return {
        "schema_version": "nursery-corrective-pristine-state-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _traceability(package: Path, snapshot: Path) -> dict:
    specification = read_json(
        snapshot / "docs/synthetic_corrective_alignment_v2_traceability.json"
    )
    expected_status = {
    ...
    specification_checks = {
        ...
        == "nursery-corrective-traceability-v2",
    ...
    return {
        "schema_version": "nursery-corrective-traceability-validation-v2",
        "status": "PASS" if passed else "FAIL",
...
def _development_command_walltime_estimate(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-development-command-walltime-v2",
...
def _validate_development_command_walltime_estimate(
    ...
    return {
        "schema_version": "nursery-corrective-development-walltime-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _validate_preservation_completion(package: Path, config: dict) -> dict:
    ...
    checks = {
        ...
        == "nursery-corrective-preservation-timing-v2"
        ...
        and completion.get("schema_version")
        == "nursery-corrective-preservation-completion-v2"
        and completion.get("status") == "PASS"
    ...
    return {
        "schema_version": "nursery-corrective-preservation-completion-validation-v2",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _scientific_report(
    ...
    report = f"""# Corrective synthetic weak-alignment study v2
