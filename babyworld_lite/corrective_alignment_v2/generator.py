...
def _rng(seed: int, label: str) -> np.random.Generator:
    digest = hashlib.sha256(f"corrective-v2|{label}|{seed}".encode()).digest()
    return np.random.default_rng(int.from_bytes(digest[:8], "big"))


def _opaque_id(seed: int, namespace: str, serial: int) -> str:
    digest = hashlib.sha256(
        f"corrective-opaque-id-v2|{seed}|{namespace}|{serial}".encode()
    ).hexdigest()
...
def _opaque_child_id(parent: str, namespace: str, serial: int) -> str:
    digest = hashlib.sha256(
        f"corrective-opaque-child-v2|{parent}|{namespace}|{serial}".encode()
    ).hexdigest()
    return f"{namespace}-{digest[:20]}"
...
def _action_geometry(
    ...
    audit = {
        "schema_version": "nursery-corrective-action-geometry-audit-v2",
...
def _side_only_probe(
    ...
    return {
        "schema_version": "nursery-corrective-side-only-probe-v2",
...
def _evaluation(
    ...
    for slot, dimension in (("primitive", primitives), ("manner", manners)):
        for concept in range(dimension):
            ...
            for repetition in range(lexical_count):
                ...
                visible.append(
                    {
                        "schema_version": "nursery-corrective-eval-visible-v2",
                        "prompt_id": prompt_id,
                        "kind": "lexical",
                        "slot": slot,
                        "tokens": [{"slot": slot, "token": token}],
                        "candidates": candidates,
                        "generator_provenance": "lexical-eval-v2",
                    }
                )
                keys.append(
                    {
            ...
            visible.append(
                {
                    ...
                    "candidates": candidates,
                    "generator_provenance": "composition-eval-v2",
                }
            )
            keys.append(
                {
            ...
            visible.append(
                {
                    ...
                    "null_option": True,
                    "generator_provenance": "presence-eval-v2",
                }
            )
            keys.append(
                {
    ...
    zero_rng = _rng(corpus_seed, "zero-exposure-eval-v2")
        ...
        visible.append(
            {
                ...
                "candidates": candidates,
                "generator_provenance": "zero-exposure-eval-v2",
            }
        )
        keys.append(
            {
...
def generate_corpus(
    ...
    for composition in training:
        ...
        for repetition in range(repetitions):
            ...
            visible.append(
                {
                    "schema_version": "nursery-corrective-train-visible-v2",
    ...
    return {
        "schema_version": "nursery-corrective-corpus-v2",
        "corpus_seed": int(corpus_seed),
        "learner_input": {
            "schema_version": "nursery-corrective-learner-input-v2",
            "training_episodes": training_visible,
            "condition_evidence": conditions,
            "evaluation_prompts": eval_visible,
        },
        "protected_adjudication": {
            "schema_version": "nursery-corrective-protected-v2",
            "corpus_seed": int(corpus_seed),
