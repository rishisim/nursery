...
def _summarize_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "multiclass_log_loss": float(
            np.mean([-math.log(float(row["correct_probability"])) for row in rows])
        ),
...
def score_predictions(
    ...
    for prompt_id in sorted(key_by_id):
        ...
        if (
            ...
            or not np.all(np.isfinite(np.asarray(scores, dtype=np.float64)))
        ...
        expected_probabilities = np.exp(
            np.clip(score_array - float(score_array.max()), -60.0, 60.0)
        )
        expected_probabilities /= expected_probabilities.sum()
        if not np.allclose(
            probabilities, expected_probabilities, rtol=0.0, atol=1e-12
        ):
            raise ValueError("prediction probabilities are not bound to scores")
    ...
    return {
        "schema_version": "nursery-corrective-score-v3",
        "by_kind": by_kind,
    ...
    *,
    ...
    for (corpus_seed, condition), values in sorted(grouped.items()):
        ...
        for name in metric_names:
            if "." in name:
                kind, field = name.split(".", 1)
                observations = [
                    float(row["metrics"]["by_kind"][kind][field])
                    for row in ordered
                ]
            else:
    ...
    return {
        "schema_version": "nursery-corrective-model-average-v3",
...
def analyze_informativeness(
    ...
    *,
    ...
    for endpoint in endpoints:
        ...
        nonperfect_fraction = float(
            np.mean(np.asarray(composite_values, dtype=float) < 1.0 - 1e-12)
        )
        composite_headroom[endpoint] = {
            "estimand_comparator": "per_corpus_max_absent_shuffled",
            "mean": composite_mean,
            "maximum_allowed": float(
                config["qualification_gates"]["maximum_negative_control_top1"]
            ),
            "mean_wrong_or_tied_fraction": 1.0 - composite_mean,
            "minimum_mean_wrong_or_tied_fraction": float(
                config["qualification_gates"][
                    "minimum_negative_control_wrong_or_tied_fraction"
                ]
            ),
            "nonperfect_corpus_fraction": nonperfect_fraction,
            "minimum_nonperfect_corpus_fraction": float(
                config["qualification_gates"][
                    "minimum_primary_comparator_nonperfect_corpus_fraction"
                ]
            ),
            ...
            "passed": (
                ...
                <= float(
                    config["qualification_gates"][
                        "maximum_negative_control_top1"
                    ]
                )
                and 1.0 - composite_mean
                >= float(
                    config["qualification_gates"][
                        "minimum_negative_control_wrong_or_tied_fraction"
                    ]
                )
                and nonperfect_fraction
                >= float(
                    config["qualification_gates"][
                        "minimum_primary_comparator_nonperfect_corpus_fraction"
                    ]
    ...
    details["primary_comparator_headroom"] = composite_headroom
    gates["negative_controls_retain_headroom"] = all(
        row["passed"]
        for endpoint in negative_headroom.values()
        for row in endpoint.values()
    ) and all(row["passed"] for row in composite_headroom.values())
    ...
    presence_state_rows = {}
    for condition, threshold_name in (
        ("synchronized", "minimum_active_presence_state_accuracy"),
        ("oracle_alignment", "minimum_oracle_presence_state_accuracy"),
    ):
        threshold = float(config["qualification_gates"][threshold_name])
        condition_rows = {}
        for state in ("present", "null"):
            values = [
                float(table[(seed, condition)][f"presence.{state}.strict_top1"])
                for seed in corpora
            ]
            summary = paired_t_summary(values, confidence=confidence)
            condition_rows[state] = {
                **summary,
                "minimum_mean_accuracy": threshold,
                "passed": summary["mean"] >= threshold,
            }
        presence_state_rows[condition] = condition_rows
    details["presence_state_accuracy"] = presence_state_rows
    gates["active_and_oracle_presence_states_are_informative"] = all(
        row["passed"]
        for condition in presence_state_rows.values()
    ...
    realized_target_lag_rms = {
        seed: float(audits[seed]["target_lag_rms"]) for seed in corpora
    }
    lag_rows = {}
    for endpoint in endpoints:
        latent_recovery = paired_t_summary(
            [
                float(table[(seed, "synchronized")][endpoint])
        ...
        recovery_pass = latent_recovery["one_sided_lcb"] > float(
            config["qualification_gates"][
                "minimum_synchronized_minus_exact_window_lcb"
            ]
        )
        lag_rows[endpoint] = {
            "synchronized_minus_exact_window": {
                **latent_recovery,
                "passed": recovery_pass,
            },
        }
    details["temporal_uncertainty"] = {
        "realized_speech_to_target_lag_rms": realized_target_lag_rms,
        "exact_window_is_prespecified_comparator": True,
        "endpoints": lag_rows,
    }
    gates["latent_alignment_outperforms_exact_window"] = all(
        row["passed"]
    ...
    null_side_rows = mutation_summary["mutations"]["presence_side_disconnected"][
        "corpus_rows"
    ]
    ...
    null_side_margin = float(
        config["qualification_gates"]["maximum_cross_path_mean_top1_effect"]
    )
    null_side_interval_margin = float(
        config["qualification_gates"][
            "presence_side_semantic_equivalence_margin"
        ]
    )
    for field in ("lexical_change", "composition_change"):
        ...
        mean_absolute_effect = float(
            np.mean([abs(float(row[field])) for row in null_side_rows])
        )
        passed = (
            interval.get("status") == "PASS"
            and float(interval["lower"]) >= -null_side_interval_margin
            and float(interval["upper"]) <= null_side_interval_margin
            and mean_absolute_effect <= null_side_margin
        )
        null_side_equivalence[field] = {
            **interval,
            "mean_absolute_cross_path_effect": mean_absolute_effect,
            "maximum_mean_absolute_cross_path_effect": null_side_margin,
            "equivalence_margin": null_side_interval_margin,
            "passed": passed,
        }
    null_side_pass = (
        ...
        > float(
            config["qualification_gates"][
                "minimum_presence_side_disconnect_drop_lcb"
            ]
        )
    ...
    details["presence_side_disconnected"] = {
        "active_minus_presence_side_disconnected_null": null_side_drop,
        "presence_side_disconnected_minus_active_present": null_side_present,
        "semantic_endpoint_equivalence": null_side_equivalence,
        "cross_path_mean_effect_margin": null_side_margin,
        "passed": null_side_pass,
    }
    gates["presence_side_path_is_selectively_causal"] = null_side_pass
    ...
    null_update_interval_margin = float(
        config["qualification_gates"][
            "null_update_semantic_equivalence_margin"
        ]
    )
    for field in ("lexical_change", "composition_change"):
        interval = equivalence_interval(
        ...
        mean_absolute_effect = float(
            np.mean([abs(float(row[field])) for row in null_update_rows])
        )
        passed = (
            interval.get("status") == "PASS"
            and float(interval["lower"]) >= -null_update_interval_margin
            and float(interval["upper"]) <= null_update_interval_margin
            and mean_absolute_effect <= null_update_margin
        )
        null_update_equivalence[field] = {
            **interval,
            "mean_absolute_cross_path_effect": mean_absolute_effect,
            "maximum_mean_absolute_cross_path_effect": null_update_margin,
            "equivalence_margin": null_update_interval_margin,
            "passed": passed,
        }
    null_update_pass = (
    ...
    details["null_update_disconnect"] = {
        ...
        "cross_path_mean_effect_margin": null_update_margin,
    ...
    semantic_null_mean_effect = float(
        np.mean([abs(float(row["null_change"])) for row in semantic_side_rows])
    )
    semantic_null_pass = (
        ...
        and semantic_null_mean_effect
        <= float(
            config["qualification_gates"][
                "maximum_cross_path_mean_top1_effect"
            ]
        )
    )
    ...
    details["semantic_side_zero"] = {
        ...
        "null_endpoint_equivalence": {
            **semantic_null_interval,
            ...
            "mean_absolute_cross_path_effect": semantic_null_mean_effect,
            "maximum_mean_absolute_cross_path_effect": float(
                config["qualification_gates"]["maximum_cross_path_mean_top1_effect"]
            ),
            "passed": semantic_null_pass,
        },
        "null_cross_path_is_required_for_selectivity": True,
    ...
    structural_gate_names = {
        ...
        "presence_side_path_is_selectively_causal",
    ...
    causal_attribution_gate_names = {
        ...
        "latent_alignment_outperforms_exact_window",
    ...
    required_gate_names = sorted(structural_gate_names)
    ...
    return {
        "schema_version": "nursery-corrective-informativeness-v3",
        "status": status,
        "purpose": purpose,
        "independent_unit": "corpus_seed",
...
def analyze_primary(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-primary-inference-v3",
...
def dependence_diagnostics(
    ...
    return {
        "schema_version": "nursery-corrective-presence-null-dependence-v3",
        "status": "PASS" if passed else "FAIL",
