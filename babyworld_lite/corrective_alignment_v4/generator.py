...
def _rng(seed: int, label: str) -> np.random.Generator:
    digest = hashlib.sha256(f"corrective-v3|{label}|{seed}".encode()).digest()
    return np.random.default_rng(int.from_bytes(digest[:8], "big"))


def _opaque_id(seed: int, namespace: str, serial: int) -> str:
    digest = hashlib.sha256(
        f"corrective-opaque-id-v3|{seed}|{namespace}|{serial}".encode()
    ).hexdigest()
...
def _opaque_child_id(parent: str, namespace: str, serial: int) -> str:
    digest = hashlib.sha256(
        f"corrective-opaque-child-v3|{parent}|{namespace}|{serial}".encode()
    ).hexdigest()
    return f"{namespace}-{digest[:20]}"
...
def _action_geometry(
    ...
    audit = {
        "schema_version": "nursery-corrective-action-geometry-audit-v3",
...
def _foil_map(
    ...
    receivers = sorted(map(tuple, compositions))
    adjacency = {
        receiver: sorted(
            [
                candidate
                for candidate in receivers
                if candidate[0] != receiver[0] and candidate[1] != receiver[1]
            ],
            key=lambda candidate: hashlib.sha256(
                f"foil-v3|{seed}|{receiver}|{candidate}".encode()
            ).hexdigest(),
        )
    ...
    donor_to_receiver: dict[tuple[int, int], tuple[int, int]] = {}

    def assign(receiver: tuple[int, int], seen: set[tuple[int, int]]) -> bool:
    ...
    ordered_receivers = sorted(
        receivers, key=lambda value: (len(adjacency[value]), value)
    )
    if not all(assign(receiver, set()) for receiver in ordered_receivers):
        raise RuntimeError("no balanced different-primitive-and-manner foil matching")
    output = {
        receiver: donor for donor, receiver in donor_to_receiver.items()
    }
    if set(output) != set(receivers) or len(set(output.values())) != len(receivers):
        raise RuntimeError("foil matching is not a bijection")
    return output
...
def _event_intervals(
    ...
    start_low, start_high = map(int, design["initial_event_start_range"])
    shift = int(design["signed_shift_samples"])
    background_start = int(design["background_proposal_start"])
    latest_event_end = background_start - shift - 2
    if start_low <= shift:
        raise RuntimeError("initial event support lacks negative-shift guard band")
    for _attempt in range(512):
        widths = rng.integers(width_low, width_high + 1, size=count)
        gaps = rng.integers(gap_low, gap_high + 1, size=max(0, count - 1))
        cursor = int(rng.integers(start_low, start_high + 1))
        output = []
        for index, width in enumerate(widths):
            end = cursor + int(width) - 1
            output.append((cursor, end))
            if index < len(gaps):
                cursor = end + 1 + int(gaps[index])
        if output[-1][1] <= latest_event_end:
            return output
    raise RuntimeError("could not construct guarded event proposals within frozen ranges")


def _matched_background_intervals(
    intervals: Sequence[tuple[int, int]],
    config: Mapping[str, Any],
) -> list[tuple[int, int]]:
    if not intervals:
        raise ValueError("matched background proposals require event proposals")
    design = config["design"]
    background_start = int(design["background_proposal_start"])
    origin = int(intervals[0][0])
    output = [
        (
            background_start + int(start) - origin,
            background_start + int(end) - origin,
        )
        for start, end in intervals
    ]
    shift = int(design["signed_shift_samples"])
    samples = int(design["raw_sensor_samples"])
    if output[-1][1] + shift >= samples:
        raise RuntimeError("background proposals lack positive-shift guard band")
    if output[0][0] - shift <= intervals[-1][1]:
        raise RuntimeError("shifted background proposals overlap event proposals")
    if [end - start + 1 for start, end in output] != [
        end - start + 1 for start, end in intervals
    ]:
        raise RuntimeError("background proposal widths do not match event proposals")
    return output
...
def _make_side_stream(
    *,
    ...
    null_background_index: int | None,
    informativity: float,
    noise: float,
    ...
    if target_index is not None and correct:
    ...
    elif target_index is None and correct:
        if null_background_index is None:
            raise ValueError("null stream requires a background target proposal")
        start, end = background_intervals[int(null_background_index)]
        positions = list(range(start, end + 1))
        ...
        if alternatives:
            ...
            start, end = background_intervals[0]
            positions = list(range(start, end + 1))
...
def detect_event_or_null(
    ...
    background_intervals: Sequence[Sequence[int]],
) -> dict[str, Any]:
    count = len(intervals)
    if count == 0 or len(background_intervals) != count:
        raise ValueError("event and matched-background proposal counts must agree")
    if [int(end) - int(start) + 1 for start, end in intervals] != [
        int(end) - int(start) + 1 for start, end in background_intervals
    ]:
        raise ValueError("event and matched-background proposal widths must agree")
    if stream is None:
        return {
            ...
            "matched_background_proposal_count": count,
        }
    ...
    event_values = np.asarray(event_energy, dtype=np.float64)
    background_values = np.asarray(background_energy, dtype=np.float64)
    event_center = float(np.mean(event_values))
    event_scale = float(np.std(event_values))
    if event_scale < 1e-8:
        event_logits = np.zeros_like(event_values)
    else:
        event_logits = np.clip(
            (event_values - event_center) / event_scale,
            -4.0,
            4.0,
        )
    pooled_scale = max(
        float(np.std(np.concatenate([event_values, background_values]))),
        1e-6,
    )
    null_logit = float(
        np.clip(
            (
                float(np.max(background_values))
                - float(np.max(event_values))
            )
            / pooled_scale,
            -6.0,
            6.0,
    ...
    return {
        "event_logits": list(map(float, event_logits)),
...
def _corrupt_evidence(
    ...
    if not events:
        raise RuntimeError("corrupted detector requires at least one event proposal")
    if target_index is None:
        # Force a false-present decision.  Event ranking is immaterial when the
        # true state is null, so preserve it and invert the independent presence
        # contrast with a strict margin.
        corrupted_events = events
        corrupted_null = -6.0
        corruption = "force_false_present_in_factorized_presence_head"
    else:
        # Keep the independent presence head strictly present while forcing a
        # unique wrong conditional event.  This avoids the invalid v3 practice
        # of comparing event logits and the null logit on one shared scale.
        correct = int(target_index)
        wrong = (correct + 1) % len(events)
        if wrong == correct:
            raise RuntimeError("corrupted detector requires two event proposals")
        ranked = sorted(events, reverse=True)
        corrupted_events = [float(value) for value in ranked[1:]]
        if len(corrupted_events) != len(events) - 1:
            raise RuntimeError("corrupted event-rank construction failed")
        output_events = [0.0] * len(events)
        output_events[wrong] = float(ranked[0]) + 1.0
        remaining = [index for index in range(len(events)) if index != wrong]
        for rank, (position, value) in enumerate(
            zip(remaining, corrupted_events), start=1
        ):
            output_events[position] = float(value) - rank * 1e-6
        corrupted_events = output_events
        corrupted_null = -6.0
        corruption = "force_unique_wrong_conditional_event_with_present_state"
    return {
        "event_logits": corrupted_events,
        "null_logit": corrupted_null,
        "detector_input_present": True,
        "factorized_presence_event_logits": True,
        "corruption": corruption,
        "matched_background_proposal_count": len(events),
    }
...
def _condition_evidence(
    ...
    for row in visible:
        synchronized[str(row["episode_id"])] = detect_event_or_null(
            ...
            row["background_proposal_intervals"],
        )
    ...
    for episode_id, row in by_id.items():
        ...
        background_intervals = row["background_proposal_intervals"]
        key = keys[episode_id]
        ...
        output["shift_minus"][episode_id] = detect_event_or_null(
            _shift_stream(row["side_stream"], -shift),
            intervals,
        ...
        output["absent"][episode_id] = detect_event_or_null(
            None,
            intervals,
            background_intervals,
        )
        output["uninformative"][episode_id] = detect_event_or_null(
            [0.0] * len(row["side_stream"]),
            intervals,
            background_intervals,
        )
        ...
        output["oracle_alignment"][episode_id] = {
            ...
            "detector_input_present": False,
            "factorized_presence_event_logits": True,
            "matched_background_proposal_count": len(row["events"]),
    ...
    for condition, rows in output.items():
        ...
        for episode_id, detector in rows.items():
            event_values = np.asarray(detector["event_logits"], dtype=float)
            target = keys[episode_id]["target_event_index"]
            if condition == "exact_window":
                speech_time = float(by_id[episode_id]["utterance"]["speech_time"])
                inside = [
                    index
                    for index, event in enumerate(by_id[episode_id]["events"])
                    if float(event["start"]) <= speech_time <= float(event["end"])
                ]
                if inside:
                    chosen = min(
                        inside,
                        key=lambda index: abs(
                            speech_time
                            - (
                                float(by_id[episode_id]["events"][index]["start"])
                                + float(by_id[episode_id]["events"][index]["end"])
                            )
                            / 2.0
                        ),
                    )
                    exact_correct = target is not None and int(chosen) == int(target)
                else:
                    exact_correct = target is None
                is_correct = int(exact_correct)
                loss = -math.log(1.0 - 1e-15 if is_correct else 1e-15)
            else:
                maximum = float(event_values.max())
                tied = np.flatnonzero(
                    np.abs(event_values - maximum) <= 1e-12
                )
                null_logit = float(detector["null_logit"])
                null_probability = 1.0 / (
                    1.0 + math.exp(-float(np.clip(null_logit, -40.0, 40.0)))
                )
                centered = event_values - maximum
                event_probabilities = np.exp(np.clip(centered, -60.0, 60.0))
                event_probabilities /= event_probabilities.sum()
                if target is None:
                    is_correct = int(null_logit > 0.0)
                    loss = -math.log(max(null_probability, 1e-15))
                else:
                    target_index = int(target)
                    is_correct = int(
                        null_logit < 0.0
                        and len(tied) == 1
                        and int(tied[0]) == target_index
                    )
                    loss = -math.log(max(1.0 - null_probability, 1e-15))
                    loss -= math.log(
                        max(float(event_probabilities[target_index]), 1e-15)
                    )
            correct += is_correct
            log_losses.append(loss)
            state = "null" if target is None else "present"
        ...
        detector_manipulation[condition] = {
            ...
            "scoring_contract": (
                "factorized_binary_presence_then_conditional_event"
                if condition != "exact_window"
                else "speech_window_event_or_null_comparator"
            ),
            "by_state": {
                state: {
    ...
    audit = {
        ...
        "detector_scoring_factorizes_presence_from_conditional_event": True,
        "event_and_null_logits_never_jointly_ranked": True,
...
def _side_only_probe(
    ...
    return {
        "schema_version": "nursery-corrective-side-only-probe-v3",
...
def _evaluation(
    ...
    for slot, dimension in (("primitive", primitives), ("manner", manners)):
        for concept in range(dimension):
            ...
            for repetition in range(lexical_count):
                ...
                visible.append(
                    {
                        "schema_version": "nursery-corrective-eval-visible-v3",
                        "prompt_id": prompt_id,
                        "kind": "lexical",
                        "slot": slot,
                        "tokens": [{"slot": slot, "token": token}],
                        "candidates": candidates,
                        "generator_provenance": "lexical-eval-v3",
                    }
                )
                keys.append(
                    {
            ...
            visible.append(
                {
                    ...
                    "candidates": candidates,
                    "generator_provenance": "composition-eval-v3",
                }
            )
            keys.append(
                {
            ...
            visible.append(
                {
                    ...
                    "null_option": True,
                    "generator_provenance": "presence-eval-v3",
                }
            )
            keys.append(
                {
    ...
    zero_rng = _rng(corpus_seed, "zero-exposure-eval-v3")
        ...
        visible.append(
            {
                ...
                "candidates": candidates,
                "generator_provenance": "zero-exposure-eval-v3",
            }
        )
        keys.append(
            {
...
def generate_corpus(
    ...
    correction_challenge_counts: dict[str, int] = {}
    episode_serial = 0
    ...
    for composition in training:
        ...
        challenge_repetitions = min(
            repetitions - int(
                design[
                    "minimum_present_and_null_episodes_per_training_composition"
                ]
            ),
            max(
                int(
                ...
                int(round(repetitions * grounded_rate)),
            ),
        )
        correction_challenge_counts[
            f"{composition[0]}|{composition[1]}"
        ...
        for repetition in range(repetitions):
            ...
            correction_challenge = repetition < challenge_repetitions
            grounded = correction_challenge
            candidates: list[tuple[int, int]] = [tuple(composition)] if grounded else []
            if (correction_challenge or episode_rng.random() < foil_rate) and foil[
                tuple(composition)
            ] not in candidates:
            ...
            pool = [
                ...
                and (grounded or value != tuple(composition))
            ]
            ...
            background_intervals = _matched_background_intervals(
                intervals, config
            )
            null_background_index = (
                None
                if target_index is not None
                else int(episode_rng.integers(0, len(background_intervals)))
            )
            ...
            stream = _make_side_stream(
                ...
                null_background_index=null_background_index,
                informativity=side_informativity,
                noise=side_noise,
                peak_range=peak_range,
                rng=episode_rng,
            )
            if correction_challenge:
                foil_index = candidates.index(foil[tuple(composition)])
                start, end = intervals[int(foil_index)]
                anchor = (start + end) / 2
                requested_anchor_lag = float(
                    episode_rng.normal(lag_mean, lag_sd)
                )
                maximum_inside = max(
                    0.0,
                    (float(end) - float(start)) / 2.0
                    - float(design["speech_anchor_inside_foil_margin_samples"]),
                )
                realized_anchor_lag = float(
                    np.clip(requested_anchor_lag, -maximum_inside, maximum_inside)
                )
            elif target_index is None:
                if null_background_index is None:
                    raise RuntimeError("null episode lacks background proposal")
                start, end = background_intervals[null_background_index]
                anchor = float((start + end) / 2)
                requested_anchor_lag = float(
                    episode_rng.normal(lag_mean, lag_sd)
                )
                maximum_inside = max(
                ...
                raise RuntimeError("every grounded episode must be a correction challenge")
            speech_time = anchor + realized_anchor_lag
            target_center = (
                None
                if target_index is None
                else float(
                    sum(intervals[int(target_index)]) / 2.0
                )
            )
            foil_center = (
                float(
                    sum(
                        intervals[
                            candidates.index(foil[tuple(composition)])
                        ]
                    )
                    / 2.0
                )
                if foil[tuple(composition)] in candidates
                else None
            )
            visible.append(
                {
                    "schema_version": "nursery-corrective-train-visible-v3",
                    ...
                    "events": events,
                    "background_proposal_intervals": [
                        list(value) for value in background_intervals
                    ],
                    "side_stream": stream,
                }
            )
            oracle.append(
                {
                    ...
                    "grounded": grounded,
                    "null_background_index": null_background_index,
                    "requested_anchor_lag": requested_anchor_lag,
                    "realized_anchor_lag": realized_anchor_lag,
                    "speech_to_target_lag": (
                        None if target_center is None else speech_time - target_center
                    ),
                    "speech_to_foil_lag": (
                        None if foil_center is None else speech_time - foil_center
                    ),
                    "speech_to_background_lag": (
                        realized_anchor_lag if target_index is None else None
                    ),
    ...
    for row in visible:
        ...
        copied.pop("background_proposal_intervals")
    ...
    target_lag_values = np.asarray(
        [
            float(key["speech_to_target_lag"])
            for key in oracle
            if key["speech_to_target_lag"] is not None
        ],
        dtype=float,
    )
    requested_anchor_lags = np.asarray(
        [float(key["requested_anchor_lag"]) for key in oracle], dtype=float
    )
    realized_anchor_lags = np.asarray(
        [float(key["realized_anchor_lag"]) for key in oracle], dtype=float
    )
    all_event_widths = [
        int(event["end"]) - int(event["start"]) + 1
        for row in visible
        for event in row["events"]
    ]
    all_event_gaps = [
    ...
    shift = int(design["signed_shift_samples"])
    samples = int(design["raw_sensor_samples"])
    matched_background_banks = all(
        len(row["background_proposal_intervals"]) == len(row["events"])
        and [
            int(end) - int(start) + 1
            for start, end in row["background_proposal_intervals"]
        ]
        == [
            int(event["end"]) - int(event["start"]) + 1
            for event in row["events"]
    ...
    shift_guard_bands_valid = all(
        int(row["events"][0]["start"]) - shift >= 0
        and int(row["events"][-1]["end"]) + shift
        < int(row["background_proposal_intervals"][0][0])
        and int(row["background_proposal_intervals"][0][0]) - shift
        > int(row["events"][-1]["end"])
        and int(row["background_proposal_intervals"][-1][1]) + shift < samples
        for row in visible
    )
    ...
    audit = {
        ...
        "observed_lag_mean": float(target_lag_values.mean()),
        "observed_lag_sd": float(target_lag_values.std(ddof=1)),
        "target_lag_rms": float(np.sqrt(np.mean(target_lag_values**2))),
        "requested_anchor_lag_mean": float(requested_anchor_lags.mean()),
        "requested_anchor_lag_sd": float(requested_anchor_lags.std(ddof=1)),
        "realized_anchor_lag_mean": float(realized_anchor_lags.mean()),
        "realized_anchor_lag_sd": float(realized_anchor_lags.std(ddof=1)),
        ...
        "correction_challenge_count": int(
            sum(correction_challenge_counts.values())
        ),
        "correction_challenge_counts": correction_challenge_counts,
        "correction_challenge_fraction": float(
            sum(correction_challenge_counts.values()) / len(oracle)
        ),
        "correction_challenges_cover_every_training_composition": all(
            value > 0 for value in correction_challenge_counts.values()
        ),
        "all_grounded_episodes_are_correction_challenges": all(
            bool(key["correction_challenge"]) == bool(key["grounded"])
            for key in oracle
        ),
        "every_training_composition_has_present_and_null_support": all(
            any(
                tuple(key["composition"]) == composition and bool(key["grounded"])
                for key in oracle
            )
            and any(
                tuple(key["composition"]) == composition and not bool(key["grounded"])
                for key in oracle
            )
            for composition in training
        ),
        "foil_map_is_bijective": len(set(foil.values())) == len(foil),
        "foil_map_changes_both_action_dimensions": all(
            source[0] != target[0] and source[1] != target[1]
            for source, target in foil.items()
        ),
        "matched_background_proposal_banks": matched_background_banks,
        "event_and_background_proposal_counts_and_widths_match": (
            matched_background_banks
        ),
        "signed_shift_guard_bands_valid": shift_guard_bands_valid,
        "realized_event_width_minimum": min(all_event_widths),
        "realized_event_width_maximum": max(all_event_widths),
        "realized_inter_event_gap_minimum": min(all_event_gaps),
        "realized_inter_event_gap_maximum": max(all_event_gaps),
        "realized_event_widths_within_frozen_range": (
            min(all_event_widths) >= int(design["event_width"][0])
            and max(all_event_widths) <= int(design["event_width"][1])
        ),
        "realized_inter_event_gaps_within_frozen_range": (
            min(all_event_gaps) >= int(design["event_gap"][0])
            and max(all_event_gaps) <= int(design["event_gap"][1])
        ),
        "all_condition_evidence_factorized": all(
            evidence.get("factorized_presence_event_logits") is True
            and int(evidence.get("matched_background_proposal_count", -1))
            == len(row["events"])
            for condition_rows in conditions.values()
            for row in visible
            for evidence in [condition_rows[str(row["episode_id"])]]
        ),
    ...
    return {
        "schema_version": "nursery-corrective-corpus-v3",
        "corpus_seed": int(corpus_seed),
        "learner_input": {
            "schema_version": "nursery-corrective-learner-input-v3",
            "training_episodes": training_visible,
            "condition_evidence": conditions,
            "evaluation_prompts": eval_visible,
        },
        "protected_adjudication": {
            "schema_version": "nursery-corrective-protected-v3",
            "corpus_seed": int(corpus_seed),
