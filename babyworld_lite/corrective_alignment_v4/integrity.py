...
REQUIRED_CORRECTIVE_TEST_GROUPS = {
    "construct_mechanism_and_leakage": [
        ...
        "test_development_validity_rechecks_heterogeneity_and_model_stochasticity",
    ...
    "parallel_shards_inventory_and_interruption": [
        ...
        "test_real_launcher_preflight_uses_lexical_venv_and_resolved_base_fails",
        "test_standard_library_bootstrap_writes_bound_import_failure_receipt",
        "test_subprocess_failure_receipt_is_exact_and_mutations_fail",
    ...
    "atomic_failure_no_outcome": [
        ...
        "test_publication_rechecks_preservation_immediately_before_atomic_rename",
    ...
    "prospective_lock_and_input_commitments": [
        "test_v3_scientific_core_is_exactly_inherited_from_v2",
        "test_predecessor_v1_tree_is_fully_rehashed_and_failure_is_detected",
        "test_predecessor_v2_tree_is_fully_rehashed_and_failure_is_detected",
        "test_micro_attempt_receipt_is_semantically_validated_and_nonreplayable",
...
def _python_launcher_chain(launcher_path: str | Path) -> list[dict[str, Any]]:
    """Bind the lexical venv launcher separately from its resolved binary."""
    current = Path(launcher_path).absolute()
    seen: set[str] = set()
...
def environment_record(
    ...
    if Path(sys.executable).absolute() != executable:
        raise RuntimeError("freeze must use the configured lexical Python launcher")
    ...
    launcher_chain = _python_launcher_chain(executable)
    if (
        len(launcher_chain) < 2
        or launcher_chain[0].get("type") != "symlink"
        or launcher_chain[-1].get("type") != "regular_file"
    ):
        raise RuntimeError("configured Python must be a bound venv symlink launcher")
    venv_root = executable.parent.parent.absolute()
    pyvenv_cfg = venv_root / "pyvenv.cfg"
    pyvenv_metadata = pyvenv_cfg.lstat()
    if stat.S_ISLNK(pyvenv_metadata.st_mode) or not stat.S_ISREG(
        pyvenv_metadata.st_mode
    ):
        raise RuntimeError("pyvenv.cfg must be a regular file")
    if Path(sys.prefix).absolute() != venv_root or Path(sys.base_prefix).absolute() == venv_root:
        raise RuntimeError("configured launcher did not activate the frozen venv prefix")
    ...
    if not all(
        Path(str(row["module_file"])).resolve().is_relative_to(venv_root.resolve())
        for row in modules.values()
    ):
        raise RuntimeError("required module resolved outside the configured venv")
    native_libraries = []
    ...
    return {
        "schema_version": "nursery-corrective-environment-lock-v4",
        "python_executable": str(executable),
        "python_executable_resolved": str(executable.resolve()),
        "python_sha256": sha256_file(executable),
        "python_launcher_chain": launcher_chain,
        "python_launcher_chain_digest": canonical_digest(launcher_chain),
        "pyvenv_cfg_path": str(pyvenv_cfg),
        "pyvenv_cfg_mode": format(stat.S_IMODE(pyvenv_metadata.st_mode), "04o"),
        "pyvenv_cfg_bytes": int(pyvenv_metadata.st_size),
...
def freeze_prefreeze_evidence(package_root: str | Path) -> dict[str, Any]:
    ...
    contract = {
        "schema_version": "nursery-corrective-prefreeze-evidence-contract-v4",
...
def verify_prefreeze_evidence(package_root: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "nursery-corrective-prefreeze-evidence-contract-v4"
...
def verify_snapshot(snapshot_root: str | Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "nursery-corrective-freeze-receipt-v4",
        ...
        == sha256_file(snapshot / "configs/synthetic_corrective_alignment_v4.yaml"),
        ...
        "micro_attempt_receipt_sha256": receipt.get(
            "micro_attempt_receipt_sha256"
        )
        == sha256_file(package / "micro_attempt_consumed.json"),
        "worker_launcher_preflight_sha256": receipt.get(
            "worker_launcher_preflight_sha256"
        )
...
def compare_adjudication_trees(
    ...
    left_exact = set(left_hashes) == ADJUDICATION_RELEVANT_FILES
    right_exact = set(right_hashes) == ADJUDICATION_RELEVANT_FILES
    passed = (
        left_exact
        and right_exact
        and not mismatched
    ...
    return {
        "schema_version": "nursery-corrective-parallel-equivalence-v4",
        "status": "PASS" if passed else "FAIL",
        "adjudication_relevant_file_count": len(shared),
        "byte_identical": passed,
        "expected_file_set_complete": left_exact and right_exact,
        "missing_required_from_jobs_1": sorted(
            ADJUDICATION_RELEVANT_FILES - set(left_hashes)
        ),
        "missing_required_from_jobs_n": sorted(
            ADJUDICATION_RELEVANT_FILES - set(right_hashes)
        ),
        "unexpected_in_jobs_1": sorted(
            set(left_hashes) - ADJUDICATION_RELEVANT_FILES
        ),
        "unexpected_in_jobs_n": sorted(
            set(right_hashes) - ADJUDICATION_RELEVANT_FILES
        ),
...
def benchmark_report(
    *,
    ...
    return {
        "schema_version": "nursery-corrective-parallel-benchmark-v4",
...
def validate_benchmark_report(
    ...
    *,
    ...
    checks = {
        ...
        == "nursery-corrective-parallel-benchmark-v4",
    ...
    return {
        "schema_version": "nursery-corrective-benchmark-validation-v4",
...
def _hash_preserved_row(arguments: tuple[Path, str, str, int, int]) -> dict[str, Any]:
    ...
    try:
        safe_relative = relative.removeprefix("./")
        data, metadata = read_confined_stable_regular_bytes(root, safe_relative)
    except FileNotFoundError:
        return {"relative": relative, "problem": "missing"}
    except (InterruptedError, OSError, PermissionError, RuntimeError, ValueError) as error:
        return {
            "relative": relative,
            "problem": "unsafe_or_unstable",
...
def verify_prior_preservation(
    ...
    *,
    ...
    proof = {
        "schema_version": "nursery-corrective-preservation-proof-v4",
        "status": "PASS" if passed else "FAIL",
...
def reverify_prior_preservation(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-preservation-reverification-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
    ...
    report = {
        ...
        "evidence_bindings": {
            ...
            "config_sha256": sha256_file(
                root / "configs/synthetic_corrective_alignment_v4.yaml"
            ),
...
def validate_official_test_report(
    ...
    *,
    ...
    checks = {
        "schema": set(report) == expected_fields
        and report.get("schema_version") == "nursery-corrective-official-tests-v4",
    ...
    return {
        "schema_version": "nursery-corrective-official-tests-validation-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def recompute_comparison_value(
    ...
    return {
        "schema_version": "nursery-corrective-recompute-comparison-v4",
        "status": "PASS" if passed else "FAIL",
...
def repository_operation_census(
    ...
    predecessor_v2_relative = (
        "output/synthetic_corrective_development_launch_package_v2"
    )
    predecessor_v2_manifest_path = package / "PREDECESSOR_V2_TREE_MANIFEST.json"
    predecessor_v2_manifest = read_json(predecessor_v2_manifest_path)
    predecessor_v2_manifest_contract = (
        predecessor_v2_manifest.get("digest")
        == "f55226f79a4fdcedca32116e1590cdd640ed2332df890f60a5eb4e3777f34d22"
        and int(predecessor_v2_manifest.get("file_count", -1)) == 41
        and int(
            predecessor_v2_manifest.get("directory_count_including_root", -1)
        )
        == 24
        and int(predecessor_v2_manifest.get("regular_file_bytes", -1))
        == 3_011_243
    )
    retired_predecessor_ledgers = sorted(
        {
            ...
            *(
                f"{predecessor_v2_relative}/{row['path']}"
                for row in predecessor_v2_manifest.get("rows", [])
                if row.get("type") == "file"
                and Path(str(row.get("path", ""))).name in ledger_names
            ),
        }
    )
    ...
    passed = (
        ...
        and predecessor_v2_manifest_contract
        and len(retired_predecessor_ledgers) == 20
        and observed_retired_predecessor_ledgers == retired_predecessor_ledgers
        and package_audit["status"] == "PASS"
    )
    return {
        "schema_version": "nursery-corrective-repository-operation-census-v4",
        "status": "PASS" if passed else "FAIL",
        ...
        "retired_predecessor_v2_manifest_sha256": sha256_file(
            predecessor_v2_manifest_path
        ),
        "retired_predecessor_v2_manifest_contract": (
            predecessor_v2_manifest_contract
        ),
...
def reverify_prior_closed_line(repository_root: str | Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "nursery-corrective-prior-line-reverification-v4",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
    ...
    *,
    ...
    recomputed_construction = construction_qualification_decision(
        ...
        merged_results=read_json(
            construction_root / "adjudication/merged_worker_results.json"
        ),
    ...
    fresh_reports = {
        ...
        "PREDECESSOR_V2_FINAL_REVERIFICATION.json": (
            frozen_runner._verify_or_create_predecessor_v2_evidence(
                root,
                package,
                create=False,
            )
...
def finalize_package(
    ...
    *,
    ...
    snapshot = package / "frozen_source_snapshot"
    runner = snapshot / "scripts/run_synthetic_corrective_alignment_v4.py"
    values = {
        ...
        "resolved_consumption_root": str(
            (root / config["paths"]["authorization_consumption_root"]).resolve()
        ),
        "resolved_consumed_authorization_path": str(
            (root / config["paths"]["consumed_authorization"]).resolve()
        ),
