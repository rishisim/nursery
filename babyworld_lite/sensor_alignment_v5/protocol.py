...
def load_config(path: str | Path, *, repository_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if value["protocol"]["status"] != "frozen":
        raise ValueError("v5 excluded-smoke package configuration must be frozen")
    ...
    for relative in value["registries"]["prior_registry_files"]:
        ...
        prior.update(_integer_leaves(yaml.safe_load(prior_path.read_text()).get("seeds", {})))
    ...
    value["resolved_registries"] = {**registries, "prior_v1_v4": sorted(prior)}
    return value
        ...
        *,
        ...
        for reference in references:
            ...
            if int(reference.value) not in expected:
                ...
                break
            for forbidden in ("future_development", "confirmation_reserve", "prior_v1_v4"):
                registry = self.registries[forbidden]
                values = set(registry) if forbidden == "prior_v1_v4" else {
...
def registry_snapshot(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "prior_identifier_count": len(resolved["prior_v1_v4"]),
...
def environment_record(repository_root: str | Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "test_roots": [
            str(config["environment"]["v5_test_root"]),
            str(config["environment"]["repository_test_root"]),
        ],
        "official_commands": [
            f"PYTHONDONTWRITEBYTECODE=1 {executable} -m pytest -q {config['environment']['v5_test_root']}",
            f"PYTHONDONTWRITEBYTECODE=1 {executable} -m pytest -q {config['environment']['repository_test_root']}",
        ],
