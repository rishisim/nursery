...
def freeze_package(
    ...
    *,
    ...
    adjudication_contract = {
        ...
        "terminal_decision_function": "babyworld_lite/sensor_alignment_v5/adjudicator.py:terminal_decision",
    }
...
def _snapshot_command(
    ...
    *,
    ...
    script = snapshot / "scripts/run_synthetic_identifiability_qualification_v5.py"
...
def run_negative_recompute_tests(output_root: str | Path) -> dict[str, Any]:
    ...
    source_target = source_copy / "babyworld_lite/sensor_alignment_v5/__init__.py"
...
def run_official_tests(repository_root: str | Path, output_root: str | Path) -> dict[str, Any]:
    ...
    for test_root in test_roots:
        command = [executable, "-m", "pytest", "-q", test_root]
        completed = subprocess.run(
            command,
            cwd=root,
            env=environment,
        ...
        if completed.returncode != 0:
            break
    passed = len(results) == 2 and all(row["exit_code"] == 0 for row in results)
    report = {
        "status": "PASS" if passed else "FAIL",
        "official": True,
        "only_admissible_test_report": True,
        "alternate_or_corrected_report_allowed": False,
        "order": "v5 tests first, then complete repository tests root",
    ...
    gates.update(
        {
            ...
            "prior_v1_v4_preservation": preservation["status"] == "PASS",
            "traceability": True,
