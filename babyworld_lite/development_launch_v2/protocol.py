...
def load_config(
    ...
    *,
    ...
    for relative in value["registries"].get(
        "prior_rehearsal_registry_files", []
    ):
        prior_value = yaml.safe_load((root / str(relative)).read_text())
        prior_registries = prior_value["registries"]
        for purpose in ("fixture_rehearsal", "excluded_rehearsal"):
            prior.update(_flatten(_registry(prior_registries[purpose])))
