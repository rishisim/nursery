...
def environment_record(
    ...
    if configured_executable.is_file():
        ...
        resolution_strategy = "configured_repository_virtual_environment"
    elif active_environment_executable.is_file():
        executable = active_environment_executable
        resolution_strategy = "active_virtual_environment_fallback"
    else:
        raise FileNotFoundError(
            f"no virtual-environment launcher: {configured_executable}"
        )
    resolved_executable = executable.resolve()
    return {
        "python_executable": str(executable),
        "configured_python_executable": str(configured_executable),
...
def write_prior_preservation_baseline(
    ...
    for directory in (
        ...
        root / "babyworld_lite/development_launch_v2",
        root / "output/synthetic_development_launch_package_v2",
    ):
        paths.update(
    ...
    paths.update(
        path.relative_to(root).as_posix()
        for path in (root / "docs").glob("synthetic_development_launch_v2*")
        if path.is_file()
    )
    rows = []
    ...
    return {
        ...
        "development_launch_v2_preserved": True,
    }
...
def freeze_package(
    ...
    *,
    ...
    write_json(
        ...
        {
            "development_output_root": "output/synthetic_development_v3_one_shot",
...
def _snapshot_command(
    ...
    *,
    ...
    script = snapshot / "scripts/run_synthetic_development_launch_v3.py"
...
def run_negative_recompute_tests(output_root: str | Path) -> dict[str, Any]:
    ...
    source_target = (
        source_copy / "babyworld_lite/development_launch_v3/__init__.py"
    )
...
def finalize_package(
    ...
    one_shot_root = root / "output/synthetic_development_v3_one_shot"
    ...
    actual = {
        ...
        if path.is_file()
        and path != output / "complete_file_manifest.json"
    }
    manifest_passed = (
