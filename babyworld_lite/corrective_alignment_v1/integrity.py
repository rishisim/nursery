...
REQUIRED_CORRECTIVE_TEST_GROUPS = {
    ...
    "parallel_shards_inventory_and_interruption": [
        ...
        "test_worker_baseexception_terminates_pool_and_returns_closed_failure",
...
def environment_record(
    ...
    return {
        "schema_version": "nursery-corrective-environment-lock-v1",
...
def freeze_prefreeze_evidence(package_root: str | Path) -> dict[str, Any]:
    ...
    contract = {
        "schema_version": "nursery-corrective-prefreeze-evidence-contract-v1",
...
def verify_prefreeze_evidence(package_root: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "nursery-corrective-prefreeze-evidence-contract-v1"
...
def verify_snapshot(snapshot_root: str | Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        == "nursery-corrective-freeze-receipt-v1",
        ...
        == sha256_file(snapshot / "configs/synthetic_corrective_alignment_v1.yaml"),
...
def compare_adjudication_trees(
    ...
    return {
        "schema_version": "nursery-corrective-parallel-equivalence-v1",
...
def benchmark_report(
    *,
    ...
    estimated_seconds = float(
        (
            input_generation_wall * corpus_scaling
            + wall_jobs_n * unit_scaling
            + final_assembly_probe_wall * unit_scaling
        )
        * 1.20
    )
    max_rss_native = max(
    ...
    concurrent_rss_bytes = int(
        (max_rss_bytes * int(jobs_n) + parent_rss_bytes)
    ...
    native_threads_measured_one = bool(native_thread_counts) and all(
        value == 1 for value in native_thread_counts
    )
    ...
    gates = {
        ...
        <= available_memory_bytes * float(maximum_host_ram_fraction),
        "disk_headroom": projected_development_disk_bytes
        <= disk_free_bytes * float(maximum_current_free_disk_fraction),
        "native_threadpools_measured_one": native_threads_measured_one,
        "thread_environment_measured": thread_environment_measured,
    }
    return {
        "schema_version": "nursery-corrective-parallel-benchmark-v1",
        ...
        "estimated_development_wall_seconds_with_20_percent_contingency": estimated_seconds,
        "estimated_development_wall_hours": estimated_seconds / 3600.0,
...
def validate_benchmark_report(
    ...
    *,
    ...
    expected_fields = {
        ...
        "estimated_development_wall_seconds_with_20_percent_contingency",
        "estimated_development_wall_hours",
    ...
    estimated_seconds = (
        float(report.get("input_generation_micro_wall_seconds", 0.0))
        * corpus_scaling
        + float(report.get("jobs_n_wall_seconds", 0.0)) * unit_scaling
        + float(report.get("final_assembly_probe_micro_wall_seconds", 0.0))
        * unit_scaling
    ) * 1.20
    telemetry_one = read_json(left / "runtime/runtime_telemetry.json")
    telemetry_n = read_json(right / "runtime/runtime_telemetry.json")
    ...
    def validate_telemetry(
        ...
        return (
            ...
            and all(
                ...
                and bool(row["native_threadpools"])
                and all(
    ...
    checks = {
        ...
        == "nursery-corrective-parallel-benchmark-v1",
        ...
        == estimated_seconds
        and float(report.get("estimated_development_wall_hours", -1.0))
    ...
    return {
        "schema_version": "nursery-corrective-benchmark-validation-v1",
...
def verify_prior_preservation(
    ...
    *,
    ...
    proof = {
        "schema_version": "nursery-corrective-preservation-proof-v1",
        "status": "PASS" if passed else "FAIL",
...
def reverify_prior_preservation(
    ...
    *,
    ...
    return {
        "schema_version": "nursery-corrective-preservation-reverification-v1",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
    ...
    report = {
        ...
        "evidence_bindings": {
            ...
            "config_sha256": sha256_file(
                root / "configs/synthetic_corrective_alignment_v1.yaml"
            ),
            "runner_sha256": sha256_file(
                root / "scripts/run_synthetic_corrective_alignment_v1.py"
            ),
...
def validate_official_test_report(
    ...
    *,
    ...
    checks = {
        "schema": set(report) == expected_fields
        and report.get("schema_version") == "nursery-corrective-official-tests-v1",
    ...
    return {
        "schema_version": "nursery-corrective-official-tests-validation-v1",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def recompute_comparison_value(
    ...
    return {
        "schema_version": "nursery-corrective-recompute-comparison-v1",
        "status": "PASS" if passed else "FAIL",
...
def repository_operation_census(
    ...
    outside_package = [
        path for path in new_ledgers if not path.startswith(f"{package_relative}/")
    ]
    ...
    missing = sorted(expected - set(new_ledgers))
    unexpected = sorted(set(new_ledgers) - expected)
    ...
    return {
        "schema_version": "nursery-corrective-repository-operation-census-v1",
        "status": "PASS" if passed else "FAIL",
...
def reverify_prior_closed_line(repository_root: str | Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "nursery-corrective-prior-line-reverification-v1",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
...
def _issue_finalization_proof(
    ...
    *,
    ...
    benchmark_validation = validate_benchmark_report(
        ...
        jobs_one_root=package / "parallel_micro_attempt_4_jobs_1",
        jobs_n_root=package / "parallel_micro_attempt_4_jobs_n",
        input_root=package / "parallel_micro_attempt_4_persisted_inputs",
        config=config,
    )
...
def finalize_package(
    ...
    *,
    ...
    snapshot = package / "frozen_source_snapshot"
    runner = snapshot / "scripts/run_synthetic_corrective_alignment_v1.py"
    values = {
