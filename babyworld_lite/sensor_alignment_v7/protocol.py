...
def load_config(path: str | Path, *, repository_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if value["protocol"]["status"] not in {"pre_freeze", "frozen"}:
        raise ValueError("v7 configuration status must be pre_freeze or frozen")
    ...
    value["resolved_registries"] = {**registries, "prior_v1_v6": sorted(prior)}
    return value
        ...
        *,
        ...
        for reference in references:
            ...
            if int(reference.value) not in expected:
                ...
                break
            for forbidden in ("future_development", "confirmation_reserve", "prior_v1_v6"):
                registry = self.registries[forbidden]
                values = set(registry) if forbidden == "prior_v1_v6" else {
...
def registry_snapshot(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "prior_identifier_count": len(resolved["prior_v1_v6"]),
