...
def _rng(seed: int, label: str) -> np.random.Generator:
    integer = int.from_bytes(
        hashlib.sha256(f"nursery-v6|{seed}|{label}".encode()).digest()[:8], "little"
    )
...
def _lexicon(seed: int) -> dict[str, list[str]]:
    ...
    return {
        "primitive": [f"q6p{seed:x}{value}" for value in rng.permutation(3)],
        "manner": [f"q6m{seed:x}{value}" for value in rng.permutation(2)],
        "noun": [f"q6n{seed:x}{value}" for value in rng.permutation(6)],
    }
...
def _evaluation(seed: int, lexicon: Mapping[str, Sequence[str]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for kind, concept, tokens, full_candidates in specifications:
        for presence in ("present", "absent"):
            ...
            prompt_id = f"v6-{seed}-eval-{kind}-{concept}-{presence}"
            prompt = {
                "schema_version": "nursery-v6-language-prompt",
                "prompt_id": prompt_id,
                "kind": kind,
                "tokens": tokens,
    ...
    *,
    ...
    for family_index, family in enumerate(("action", "noun")):
        ...
        for count, indices in sorted(indices_by_count.items()):
            schedule_seed = (
                int(design["factor_schedule_seed"])
        ...
        for factors in family_specs:
                ...
                episode_id = f"v6-{corpus_seed}-{family[0]}-{concept}-{repetition:02d}"
                visible_row = {
                    "schema_version": "nursery-v6-visible",
                ...
                oracle_row = {
                    "schema_version": "nursery-v6-oracle",
                    "episode_id": episode_id,
        ...
        for group_key, indices in sorted(groups.items(), key=lambda item: str(item[0])):
            ...
            ordered = sorted(
                ...
                key=lambda index: hashlib.sha256(
                    f"v6-shuffle|{rows[index]['episode_id']}".encode()
                ).hexdigest(),
            )
