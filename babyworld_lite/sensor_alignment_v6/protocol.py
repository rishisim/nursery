...
def load_config(path: str | Path, *, repository_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if value["protocol"]["status"] not in {"pre_freeze", "frozen"}:
        raise ValueError("v6 configuration status must be pre_freeze or frozen")
    ...
    value["resolved_registries"] = {**registries, "prior_v1_v5": sorted(prior)}
    return value
        ...
        *,
        ...
        for reference in references:
            ...
            for forbidden in (
                ...
                values = set(registry) if forbidden == "prior_v1_v5" else {
...
def registry_snapshot(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "prior_identifier_count": len(resolved["prior_v1_v5"]),
...
def environment_record(repository_root: str | Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "official_commands": [
            f"PYTHONDONTWRITEBYTECODE=1 {executable} -m pytest -q {config['environment']['version_test_root']}",
