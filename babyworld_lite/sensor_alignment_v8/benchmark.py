...
def _rng(seed: int, label: str) -> np.random.Generator:
    integer = int.from_bytes(
        hashlib.sha256(f"nursery-v8|{seed}|{label}".encode()).digest()[:8], "little"
    )
...
def _lexicon(seed: int) -> dict[str, list[str]]:
    ...
    return {
        "primitive": [f"q8p{seed:x}{value}" for value in rng.permutation(3)],
        "manner": [f"q8m{seed:x}{value}" for value in rng.permutation(2)],
        "noun": [f"q8n{seed:x}{value}" for value in rng.permutation(6)],
    }
...
def _evaluation(seed: int, lexicon: Mapping[str, Sequence[str]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for kind, concept, tokens, full_candidates in specifications:
        for presence in ("present", "absent"):
            ...
            prompt_id = f"v8-{seed}-eval-{kind}-{concept}-{presence}"
            prompt = {
                "schema_version": "nursery-v8-language-prompt",
                "prompt_id": prompt_id,
                "kind": kind,
                "tokens": tokens,
...
def _assign_balanced_factor(
    ...
    *,
    ...
    """Construct a balanced factor permutation under the audited controls.

    The search is deterministic and corpus-seed invariant. It scores the exact
    noun and distractor-expanded action controls used by the persisted audit,
    plus target presence, sensor stratum, concept, and earlier nuisance factors.
    It uses only declared design-cell labels, never learner outputs or
    evaluation keys.
    """

    base = [levels[index % len(levels)] for index in range(len(indices))]
    if search_trials < 1:
        raise ValueError("factor-assignment search must execute at least one trial")
    grounded_indices = [
        index for index in indices if bool(family_specs[index]["grounded"])
    ]
    if not grounded_indices:
        raise RuntimeError(f"no grounded construction rows for {family}/{factor}")
    control_pairs: dict[str, list[tuple[int, Any]]] = {
        "all|concept": [
            (index, family_specs[index]["_concept"]) for index in indices
        ],
        "all|grounded": [
            (index, bool(family_specs[index]["grounded"])) for index in indices
        ],
        "all|sensor_stratum": [
            (index, family_specs[index]["sensor_stratum"]) for index in indices
        ],
    }
    if family == "noun":
        control_pairs.update(
            {
                "noun|target_position": [
                    (index, family_specs[index]["target_position"])
                    for index in grounded_indices
                ],
                "noun|owner_position": [
                    (index, family_specs[index]["owner_position"])
                    for index in grounded_indices
                ],
                "noun|target_is_owner": [
                    (index, family_specs[index]["_target_is_owner"])
                    for index in grounded_indices
                ],
            }
        )
    elif family == "action":
        expanded_targets: list[tuple[int, Any]] = []
        expanded_relations: list[tuple[int, Any]] = []
        for index in grounded_indices:
            spec = family_specs[index]
            candidates, target_index = _structural_candidates(
                family="action",
                concept=int(spec["_concept"]),
                count=int(spec["candidate_count"]),
                grounded=True,
                target_position=int(spec["target_position"]),
                cycle=int(spec["candidate_cycle"]),
                corpus_seed=0,
                sensor_stratum=str(spec["sensor_stratum"]),
            )
            if target_index is None:
                raise RuntimeError("grounded action construction lost its target")
            target_primitive, target_manner = _components(int(spec["_concept"]))
            for candidate_index, candidate in enumerate(candidates):
                if candidate_index == target_index:
                    continue
                primitive, manner = _components(int(candidate))
                expanded_targets.append((index, int(target_index)))
        ...
        control_pairs.update(
            {
                "action|target_position": expanded_targets,
                "action|distractor_relation": expanded_relations,
                "action|candidate_cycle": [
                    (index, family_specs[index]["candidate_cycle"])
                    for index in grounded_indices
                ],
            }
        )
    ...
    best_control_associations: dict[str, float] = {}
    best_factor_associations: dict[str, float] = {}
    trials_executed = 0
    for trial in range(search_trials):
        ...
        value_by_index = {
            index: values[position] for position, index in enumerate(indices)
        }
        control_associations: dict[str, float] = {}
        for control, selected in control_pairs.items():
            selected_pairs = [
                (left, value_by_index[index])
                for index, left in selected
                if left is not None
            ]
            if (
                ...
                continue
            control_associations[control] = _cramers_v(
                [left for left, _ in selected_pairs],
                [right for _, right in selected_pairs],
            )
        factor_associations: dict[str, float] = {}
        for previous in previously_assigned:
            selected_pairs = [
                (family_specs[index][previous], value_by_index[index])
                for index in indices
            ]
            factor_associations[previous] = _cramers_v(
                [left for left, _ in selected_pairs],
                [right for _, right in selected_pairs],
            )
        maximum_control = max(control_associations.values(), default=0.0)
        maximum_factor = max(factor_associations.values(), default=0.0)
        score = (
            max(maximum_control, maximum_factor),
            maximum_control,
            maximum_factor,
        ...
        if best_score is None or score < best_score:
            best_score = score
            best_values = values
            best_control_associations = control_associations
            best_factor_associations = factor_associations
        if score[0] <= maximum_allowed:
            best_score = score
            best_values = values
            best_control_associations = control_associations
            best_factor_associations = factor_associations
    ...
    return {
        "family": family,
        "candidate_count": int(family_specs[indices[0]]["candidate_count"]),
        "factor": factor,
        "construction_seed": int(seed),
        "maximum_allowed": float(maximum_allowed),
        "trials_executed": trials_executed,
        "maximum_trials": int(search_trials),
        "corpus_seed_invariant": True,
        "grounded_rows_scored": len(grounded_indices),
        "all_rows_balanced": len(indices),
        "maximum_audited_control_cramers_v": max(
            best_control_associations.values(), default=0.0
        ),
        "maximum_previous_factor_cramers_v": max(
            best_factor_associations.values(), default=0.0
        ),
        "audited_control_cramers_v": best_control_associations,
        "previous_factor_cramers_v": best_factor_associations,
        "assignment_digest": canonical_digest(best_values),
    }
...
def generate_corpus(
    ...
    *,
    ...
    factor_assignment_audits: list[dict[str, Any]] = []
    for family_index, family in enumerate(("action", "noun")):
        ...
        search_trials = int(design["factor_assignment_search_trials"])
        construction_bound = float(
            config["gates"]["maximum_constructed_factor_control_cramers_v"]
        )
        for count, indices in sorted(indices_by_count.items()):
            ...
            for factor, levels in factor_levels.items():
                factor_assignment_audits.append(
                    _assign_balanced_factor(
                        ...
                        maximum_allowed=construction_bound,
                    )
                )
        ...
        for factors in family_specs:
                ...
                candidates, target_index = _structural_candidates(
                    ...
                    corpus_seed=0 if family == "action" else corpus_seed,
                ...
                episode_id = f"v8-{corpus_seed}-{family[0]}-{concept}-{repetition:02d}"
                visible_row = {
                    "schema_version": "nursery-v8-visible",
                ...
                oracle_row = {
                    "schema_version": "nursery-v8-oracle",
                    "episode_id": episode_id,
    ...
    search_objective_maximum = max(
        (
            max(
                float(row["maximum_audited_control_cramers_v"]),
                float(row["maximum_previous_factor_cramers_v"]),
            )
            for row in factor_assignment_audits
        ),
        default=1.0,
    )
    construction_maximum = max(
        float(audit["maximum_factor_control_cramers_v"]),
        float(audit["maximum_pairwise_cramers_v"]),
    )
    construction_bound = float(
        config["gates"]["maximum_constructed_factor_control_cramers_v"]
    )
    construction_pass = construction_maximum <= construction_bound
    audit["factor_assignment_construction"] = {
        "status": "PASS" if construction_pass else "FAIL",
        "corpus_seed_invariant": True,
        "action_candidate_lattice_seed_invariant": True,
        "bound": construction_bound,
        "search_trials_per_assignment": int(
            design["factor_assignment_search_trials"]
        ),
        "maximum_search_objective_cramers_v": search_objective_maximum,
        "maximum_exact_persisted_audit_cramers_v": construction_maximum,
        "exact_persisted_factor_control_cramers_v": float(
            audit["maximum_factor_control_cramers_v"]
        ),
        "exact_persisted_pairwise_cramers_v": float(
            audit["maximum_pairwise_cramers_v"]
        ),
        "cells": factor_assignment_audits,
    }
    audit["status"] = (
        "PASS" if audit["status"] == "PASS" and construction_pass else "FAIL"
    )
    return Corpus(
        int(corpus_seed),
        tuple(visible),
        ...
        for group_key, indices in sorted(groups.items(), key=lambda item: str(item[0])):
            ...
            ordered = sorted(
                ...
                key=lambda index: hashlib.sha256(
                    f"v8-shuffle|{rows[index]['episode_id']}".encode()
                ).hexdigest(),
            )
