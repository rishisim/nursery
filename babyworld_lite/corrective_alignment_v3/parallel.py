...
def _verify_frozen_execution_boundary(
    ...
    frozen_config_path = snapshot / "configs/synthetic_corrective_alignment_v3.yaml"
...
def create_worker_boundary_commitment(
    *,
    ...
    payload = {
        "schema_version": "nursery-corrective-worker-boundary-v3",
        "mode": mode,
...
def verify_worker_boundary_commitment(
    ...
    *,
    ...
    if allow_historical_prefreeze_transition and commitment.get("mode") == (
        ...
        nonconfig_paths = [path for path in paths if path != "configs/synthetic_corrective_alignment_v3.yaml"]
    ...
    if (
        ...
        != "nursery-corrective-worker-boundary-v3"
...
_SUBPROCESS_REQUEST_SCHEMA = "nursery-corrective-subprocess-request-v3"
_SUBPROCESS_SUCCESS_SCHEMA = "nursery-corrective-subprocess-success-v3"
_SUBPROCESS_FAILURE_SCHEMA = "nursery-corrective-subprocess-failure-v3"
...
def _execute_worker_batches_impl(
    *,
    ...
    expected_entrypoint = repository / "scripts/run_synthetic_corrective_alignment_v3_worker.py"
    ...
    try:
        ...
        write_json(
            ...
            {
                "schema_version": "nursery-corrective-subprocess-parent-failure-v3",
                "status": "FAILED_NO_OUTCOME",
                "exception": type(error).__name__,
                "message": str(error),
...
def validate_subprocess_scheduler_artifacts(
    *,
    ...
    if (
        ...
        != "nursery-corrective-subprocess-scheduler-v3"
    ...
    report = {
        "schema_version": "nursery-corrective-scheduler-validation-v3",
        "status": "PASS",
...
def build_work_plan(config: Mapping[str, Any], *, purpose: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": "nursery-corrective-work-plan-v3",
        "purpose": purpose,
...
def _worker(
    ...
    if purpose in {"development", "excluded_rehearsal"}:
        ...
        if (
            ...
            != (snapshot / "configs/synthetic_corrective_alignment_v3.yaml").resolve()
        ):
...
def _validate_shard(
    ...
    *,
    ...
    if (
        ...
        or result.get("schema_version") != "nursery-corrective-worker-result-v3"
...
def _mutation_summary(
    ...
    return {
        "schema_version": "nursery-corrective-mutation-summary-v3",
        "mutations": output,
...
def _development_validity_audit(
    *,
    ...
    if purpose != "development":
        return {
            "schema_version": "nursery-corrective-development-validity-v3",
    ...
    if not (
        ...
        and averaged.get("schema_version") == "nursery-corrective-model-average-v2"
    ...
    for corpus_seed in expected_corpora:
        ...
        for model_seed in expected_models:
            ...
            model_state = {
                ...
                "null_rates": model.get("null_rates") if isinstance(model, Mapping) else None,
            }
            model_state_numeric = (
                isinstance(model_state["semantics"], Mapping)
                and bool(model_state["semantics"])
                and isinstance(model_state["null_rates"], Mapping)
                and set(model_state["semantics"]) == set(model_state["null_rates"])
                and all(
                ...
                and all(
                    is_finite_number(value) and 0.0 < float(value) < 1.0
                    for value in model_state["null_rates"].values()
                )
            )
            ...
            if not (
                ...
                == "nursery-corrective-training-trace-v2"
                ...
                and model.get("schema_version") == "nursery-corrective-lexicon-model-v2"
                ...
                and trace.get("null_side_weight_effective")
            ...
            canonical_digest(
                {
                    ...
                    "null_rates": row["model"]["null_rates"],
                }
            )
            for row in synchronized_rows
        ]
    ...
    checks = {
        ...
        "model_metric_variation_is_operational": nonzero_variation_fraction
        >= 0.50,
    }
    return {
        "schema_version": "nursery-corrective-development-validity-v3",
...
def run_parallel_from_persisted(
    *,
    ...
    runner_path = repository / "scripts/run_synthetic_corrective_alignment_v3.py"
    ...
    scientific_contract = {
        "schema_version": "nursery-corrective-scientific-execution-contract-v3",
        "purpose": purpose,
    ...
    runtime_contract = {
        "schema_version": "nursery-corrective-runtime-execution-contract-v3",
        "scientific_contract_digest": scientific_contract_digest,
        "jobs": int(jobs),
    ...
    write_json(
        ...
        {
            "schema_version": "nursery-corrective-scientific-summary-v3",
            "purpose": purpose,
    ...
    completeness = {
        "schema_version": "nursery-corrective-completeness-v3",
        "status": "PASS",
        "purpose": purpose,
...
def _recompute_adjudication_from_rehearsal(
    *,
    ...
    if (
        snapshot.name != "frozen_source_snapshot"
        or config_file != snapshot / "configs/synthetic_corrective_alignment_v3.yaml"
...
def _execute_development_cohort(
    *,
    ...
    _verify_frozen_execution_boundary(snapshot, config)
    expected_config_path = snapshot / "configs/synthetic_corrective_alignment_v3.yaml"
...
def _execute_excluded_rehearsal_cohort(
    *,
    ...
    snapshot = Path(repository_root).resolve()
    _verify_frozen_execution_boundary(snapshot, config)
    expected_config = snapshot / "configs/synthetic_corrective_alignment_v3.yaml"
    ...
    if (
        snapshot.name != "frozen_source_snapshot"
        or snapshot.parent.name != "synthetic_corrective_development_launch_package_v3"
