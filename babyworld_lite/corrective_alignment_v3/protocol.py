...
def load_config(
    ...
    *,
    ...
    if int(value["protocol"].get("package_version", -1)) != 3:
    ...
    scientific_sections = ["design", "learner", "analysis", "qualification_gates"]
    ...
    if scientific_core != {
        ...
        raise ValueError("scientific-core inheritance contract mismatch")
    projection = {name: value[name] for name in scientific_sections}
    if canonical_digest(projection) != expected_scientific_projection:
        raise ValueError("scientific config projection changed from retired v2")
    claim_and_leakage = {
        "protocol": {
    ...
    if canonical_digest(claim_and_leakage) != expected_claim_and_leakage:
        raise ValueError("scientific claim or leakage firewall changed from retired v2")
    for relative, expected_digest in expected_scientific_sources.items():
        if sha256_file(ensure_confined_regular_file(root, relative)) != expected_digest:
            raise ValueError(f"scientific source changed from retired v2: {relative}")
...
def issue_development_capability(
    *,
    ...
    if (
        ...
        != "nursery-corrective-authorization-consumption-v3"
    ...
    if (
        ...
        or package.name != "synthetic_corrective_development_launch_package_v3"
...
def verify_development_capability(
    ...
    *,
    ...
    if (
        ...
        != "nursery-corrective-capability-issuance-v3"
...
def registry_snapshot(config: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "nursery-corrective-registry-v3",
